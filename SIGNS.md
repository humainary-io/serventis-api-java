...
### Executable form: the `KIND` maps

The categories above roll up to a single binary at the level that matters for ascent: every sign is
either an **Operation** (an act the implementer performed) or an **Outcome** (a result it observed).
Flow Control, the act side of State Transition, Retry, Escalation, Progress, and Modification are
operations; Outcome, Error, Condition, Contention, and the result side of State Transition are
outcomes. As of Serventis 3.0 this binary is executable, and every registered domain vocabulary
publishes a `public static final SignMap<Sign, Serventis.Kind> KIND` (total, exhaustive — no
`default`), the structural twin of its `STATUS` map. `KIND` is *grammatical* (act vs result) and
deliberately independent of the *evaluative* `STATUS` reading. It describes a tendency in how a sign
carries evidence; it does **not** select which interpretation a sign feeds — operations may carry
per-sign status readings and outcomes may be structurally significant. See `Serventis.Kind` and "The
Kind of a Sign" in `SERVENTIS.md`.
...
| DENY    | Outcome   | Non-blocking request denied |
...
**Translation to Status** (pattern-derived, not a canonical map — Actors publishes `SIGNS` and
`KIND`, no `STATUS`):
Many DENY → DEGRADED, low ACKNOWLEDGE rate → DIVERGING, PROMISE without DELIVER → DEFECTIVE
(subscriber inferences over sign patterns)
...
**Translation to Status**: Many BREACH → DEGRADED, high FULFILL rate → STABLE, RETRACT signals →
ERRATIC
...
**Pattern**: Creation (SPAWN) + execution (START) + termination outcomes (STOP/FAIL/CRASH) + forced
termination (KILL) + supervision (RESTART)
...
(COMPLETE/FAIL/TIMEOUT) + cancellation (CANCEL)
...
**Pattern**: Binary outcome (MEET/MISS) × constraint type (DEADLINE/THRESHOLD). Inspired by Apdex,
provides qualitative vocabulary for time constraint outcomes without encoding quantitative
...
**Use cases**: SLA compliance, deadline tracking, Apdex-style performance classification, batch
processing deadlines, request timeout analysis

**Translation to Status**: High MISS rate → DEGRADED, sustained MEET → STABLE. The canonical map is
sign-keyed — DEADLINE vs THRESHOLD does not change the reading; a consumer that wants to weight
deadline misses more severely composes a ballot on the `Signal` rather than relying on the canonical
ascent.
...
(SPIN/YIELD/BACKOFF) + escalation behaviors (PARK/EXHAUST). Focuses on contention dynamics rather
than ownership semantics. Complements Locks API: Atomics tracks *how* contention is managed, Locks
tracks *what* ownership state results.

**Key Distinction from Locks**: Locks.CONTEST indicates CAS failure in context of lock acquisition.
Atomics provides finer-grained observability of the retry/backoff behaviors that follow contention
detection. Use together: Atomics for contention dynamics, Locks for ownership lifecycle.

**Use cases**: Lock-free data structures (Treiber stack, Michael-Scott queue), spin locks, adaptive
spinning, hybrid locks (spin-then-park), CAS-based counters, atomic reference updates

**Translation to Status**: High FAIL/ATTEMPT ratio → DEGRADED, frequent PARK → DEGRADED, EXHAUST
signals → DEFECTIVE, low FAIL rate → STABLE
...
**Translation to Status**: Canonical per-sign map — ENQUEUE → STABLE, OVERFLOW → DEGRADED; DEQUEUE
and UNDERFLOW abstain, since a consumer finding the queue empty is normal flow
...
| EVICT  | Outcome   | Entry removed due to capacity/policy |
| EXPIRE | Outcome   | Entry removed due to TTL expiration  |
| REMOVE | Operation | Entry explicitly invalidated/removed |

**Pattern**: Lookup operation (LOOKUP) + outcomes (HIT/MISS) + storage (STORE) + removal conditions
(EVICT/EXPIRE/REMOVE). Cache lifecycle from access through removal.
...
**Translation to Status**: High MISS rate → DEGRADED, many EVICT → DEGRADED, many EXPIRE → DIVERGING
...
**Pattern**: Data flow (INPUT/OUTPUT) + transformations (TRANSFORM/FILTER/AGGREGATE) + flow control
(BUFFER/BACKPRESSURE/OVERFLOW) + progress tracking (CHECKPOINT/WATERMARK) + anomalies (LAG/SKIP)
...
**Translation to Status**: Canonical per-sign map — OUTPUT → STABLE, OVERFLOW → DEGRADED, LAG →
DEGRADED, every other sign abstains. Pattern-derived readings layer on top (subscriber inferences
over sign patterns): many SKIP → DEFECTIVE
...
**Translation to Status**: Canonical per-sign map — INFO → STABLE, WARNING → DEGRADED, SEVERE →
DEFECTIVE, DEBUG abstains. Pattern-derived readings layer on top (subscriber inferences over sign
patterns): a warning crescendo (rising WARNING rate) → DIVERGING, a DEBUG flood (log-volume spike) →
ERRATIC
...
**Pattern**: Monotonic accumulation (INCREMENT) + boundary condition (OVERFLOW) + intentional reset
(RESET). Counters enforce monotonic increase - they never decrease legitimately.
...
**Translation to Status**: Many OVERFLOW → ERRATIC, high increment rate → STABLE
...
**Translation to Status** (pattern-derived, not a canonical map — Gauges publishes `SIGNS` and
`KIND`, no `STATUS`):
OVERFLOW → DEGRADED, UNDERFLOW → DEGRADED, high RESET rate → ERRATIC (subscriber inferences over
sign patterns)
...
**Translation to Status**: High DENY → DEGRADED, overload DROP → DEFECTIVE, EXPAND activity →
CONVERGING, CONTRACT + DENY → DIVERGING, DRAIN → CONVERGING
...
| CORRUPT    | Anomaly   | Packet corruption detected              |
| REORDER    | Anomaly   | Out-of-order packet arrival detected    |
...
**Translation to Status**: High DROP → DEGRADED, many CORRUPT → DEFECTIVE, high REORDER → ERRATIC
...
| TRIP      | Outcome          | Failure threshold exceeded           |
...
**Use cases**: Service protection, cascading failure prevention, resilience patterns (Resilience4j,
Hystrix)

**Translation to Status** (latest-state): OPEN → DEFECTIVE (backend predominantly failing), TRIP →
DEGRADED (break onset), sustained CLOSE → STABLE; flapping (rapid TRIP/CLOSE) → ERRATIC (a trend
reading, for the sequencer)
...
- High SUCCESS across all stages → STABLE
- High FAIL at INGRESS → DEGRADED (refusing work)
- High FAIL at TRANSIT → DEGRADED (processing failures)
- High FAIL at EGRESS → DEGRADED (output failures)
...
**Translation to Status**: High BORROW without RECLAIM → DEFECTIVE, rapid EXPAND/CONTRACT → ERRATIC,
high BORROW rate → DEGRADED
...
(RELEASE/EXPIRE/REVOKE) + monitoring (PROBE). Time-bounded resource ownership with automatic
expiration.
...
**Use cases**: Leader election, distributed locking, resource reservation, split-brain prevention,
coordination services (etcd, Zookeeper)

**Translation to Status**: Many EXPIRE without RENEW → DIVERGING, high DENY → DEGRADED, REVOKE
signals → DEFECTIVE
...
**Translation to Status**: Canonical per-sign map — PUSH → STABLE, OVERFLOW → DEGRADED; POP and
UNDERFLOW abstain. Pattern-derived readings layer on top (subscriber inferences over sign patterns):
high PUSH without POP → DIVERGING, rapid PUSH/POP → ERRATIC
...
durations → DIVERGING, frequent RESET with short phases → ERRATIC
...
perspectives. Based on REA (Resource-Event-Agent) accounting model where economic activity consists
of dual exchanges (every give implies a take). Also supports Java Exchanger synchronization
rendezvous pattern.
...
**Translation to Status** (pattern-derived, not a canonical map — Exchanges publishes `SIGNS`,
`DIMENSIONS` and `KIND`, no `STATUS`): Many CONTRACT without TRANSFER → DEGRADED, high TRANSFER rate
→ STABLE, asymmetric CONTRACT (one side only) → DIVERGING (subscriber inferences over sign patterns)
...
**Translation to Status**: Sustained ABOVE × THRESHOLD → DEGRADED, ABOVE × BASELINE + BELOW ×
THRESHOLD → DIVERGING, BELOW × TARGET → STABLE, oscillating around BASELINE → STABLE
...
### Evals API (5 signs × 8 dimensions = 40 signals)

| Sign    | Category | Notes                                             |
|---------|----------|---------------------------------------------------|
| PASS    | Outcome  | Criterion satisfied                               |
| FAIL    | Outcome  | Criterion violated                                |
| UNKNOWN | Outcome  | Evaluated, but the verdict remained indeterminate |
| SKIP    | Outcome  | Criterion deliberately not evaluated              |
| ERROR   | Outcome  | Evaluation attempted but could not complete       |

**Dimensions** (evaluation criteria):

| Dimension    | Judges  | Property judged                                       |
|--------------|---------|-------------------------------------------------------|
| CORRECTNESS  | Output  | Factual or functional correctness                     |
| COMPLETENESS | Output  | Coverage of required information or work              |
| GROUNDEDNESS | Output  | Support in authoritative evidence or supplied context |
| HELPFULNESS  | Output  | Usefulness in accomplishing the request or objective  |
| ADHERENCE    | Output  | Compliance with instructions, formats, and schemas    |
| SAFETY       | Output  | Compliance with safety requirements                   |
| TOOLING      | Process | Quality of tool selection, arguments, and use         |
| ROUTING      | Process | Quality of delegation, handoff, and context transfer  |

The criteria are deliberately non-overlapping: a single defect fires exactly one of them, so an
observer weighting the set never counts that defect twice. `HELPFULNESS` subsumes mere relevance —
an answer that does not address the request cannot be useful for it — and `ADHERENCE` excludes
safety policy, which `SAFETY` judges. A criterion is admitted only when an observer would weight it
differently from every criterion already present.

**Pattern**: A small domain-owned evaluation-result vocabulary qualified by the property judged.
Its `OUTCOME` ascent is deliberately lossy: PASS → SUCCESS, FAIL → FAIL, UNKNOWN/ERROR → UNKNOWN,
while SKIP abstains because no verdict was rendered.

**Key Characteristics**:

- **Qualitative results only**: scores, error details, provenance, and orchestration stay outside
  Evals
- **Criterion-qualified**: the same five results read consistently across every criterion
- **Explicit incompletion**: UNKNOWN means evaluated but unresolved, SKIP means deliberately not
  evaluated, ERROR means attempted but unable to complete, and no signal means no evaluation event
  was observed

**Use cases**: LLM and agent eval suites, release gating, online quality assessment, and safety or
policy verdict reporting

**Translation to Status**: PASS → STABLE, FAIL → DEGRADED; UNKNOWN, SKIP, and ERROR abstain.

**Whose subject**: Evals signals read the evaluated *unit of work* — the turn, run, or response
being judged — so its output-quality and process-quality criteria are alike properties of one
subject. The subject is never the evaluator: ERROR reports a defect in the evaluator rather than in
what it was evaluating, so it abstains from the status reading and stays out of the success/failure
tallies — a flaky judge must not degrade the subject it judges. Report evaluator health against the
evaluator's own subject.

### Messages API (7 signs × 3 dimensions = 21 signals)

| Sign      | Category  | Notes                                             |
|-----------|-----------|---------------------------------------------------|
| PUBLISH   | Operation | Message handed on for delivery                    |
| DELIVER   | Operation | Message handed to a receiver                      |
| ACK       | Outcome   | Receiving end confirmed the message               |
| NACK      | Outcome   | Receiving end refused or failed the message       |
| REDELIVER | Operation | Delivered again; earlier one refused or unsettled |
| EXHAUST   | Error     | Delivery attempts ran out; given up               |
| EXPIRE    | Error     | Time to live ran out before delivery              |

**Dimensions** (reporting party):

| Dimension | Perspective | Notes                                     |
|-----------|-------------|-------------------------------------------|
| PRODUCER  | Sending     | Party that publishes messages             |
| BROKER    | Relaying    | Intermediary that holds and delivers them |
| CONSUMER  | Receiving   | Party that receives and processes them    |

**Pattern**: Operation → outcome over a delivery contract. The transport vocabularies say something
moved; Messages says whether the receiving end accepted it. The first registered perspective set
with three members, because a brokered delivery has three parties that can each report a settlement.

**Translation to Status**: ACK → STABLE; NACK and EXPIRE → DEGRADED; EXHAUST → DEFECTIVE;
PUBLISH, DELIVER and REDELIVER abstain. No `OPERATION` map: the episode spans roles, as in Services.

**Use cases**: Broker publish confirms, consumer acknowledgement rates, poison messages reaching a
delivery limit, producer delivery timeouts

### Members API (7 signs × 2 dimensions = 14 signals)

| Sign    | Category         | Notes                                    |
|---------|------------------|------------------------------------------|
| JOIN    | State Transition | Member joined the group                  |
| LEAVE   | State Transition | Member left gracefully                   |
| SUSPECT | Condition        | Failure detector marked a member suspect |
| REFUTE  | Outcome          | Suspected member proved it was alive     |
| EVICT   | Error            | Member declared failed and removed       |
| ELECT   | Outcome          | Member became leader                     |
| RESIGN  | State Transition | Leader stepped down                      |

**Dimensions** (whose event):

| Dimension | Perspective | Notes                                     |
|-----------|-------------|-------------------------------------------|
| SELF      | Concerned   | Reported by the member the event concerns |
| PEER      | Observing   | Reported by another member of the group   |

**Pattern**: Membership and leadership as a membership protocol decides them. Primary only because
`SUSPECT` and `EVICT` report the failure detector's own decision; a producer computing suspicion
from
heartbeat gaps itself would be emitting a derived observation the vocabulary does not carry.

**Translation to Status**: REFUTE and ELECT → STABLE; SUSPECT → DEGRADED from either end. EVICT
abstains: a sign-keyed map cannot tell a routine peer eviction from this member's own removal while
still running, which is how split brain first shows. A consumer keys on the full signal for that.

**Use cases**: Gossip and SWIM failure detection, Raft and ZooKeeper leadership changes, cluster
membership churn, split-brain detection

### Changes API (6 signs, no dimensions)

| Sign     | Category         | Notes                              |
|----------|------------------|------------------------------------|
| START    | State Transition | Change began, validation included  |
| APPLY    | Outcome          | Change took effect                 |
| FAIL     | Outcome          | Change could not be applied        |
| REJECT   | Outcome          | Subject refused the change         |
| REVERT   | Outcome          | Rolled back before it took effect  |
| PROGRESS | Progress         | Advanced a stage, such as a canary |

**Pattern**: One episode per change: START opens it, PROGRESS marks each stage of a staged rollout,
and APPLY, FAIL, REJECT or REVERT closes it. REVERT is a result, as a transaction's ROLLBACK is, and
reads as a failure in outcome tallies. Validation is part of the change, so a refusal before rollout
still follows START. Undoing a change
that already took effect is a new change, so no sign arrives after its episode has closed. A subject
carries one open change at a time; changes that can overlap go under distinct subjects, and a change
abandoned for a newer one is closed first. It brackets cleanly with
`Sequencers.flow ( Changes.OPERATION, Changes.STATUS )`.

**Translation to Status**: APPLY → STABLE; FAIL and REVERT → DEGRADED; START, PROGRESS and REJECT
abstain, since refusing a bad change is the system working.

**Use cases**: Deployments, configuration and feature-flag changes, schema migrations, correlating a
degraded subject with what changed

### Guards API (4 signs × 2 dimensions = 8 signals)

| Sign      | Category  | Notes                           |
|-----------|-----------|---------------------------------|
| ATTEMPT   | Operation | Credential or request presented |
| CHALLENGE | Operation | Further proof demanded          |
| GRANT     | Outcome   | Access allowed                  |
| DENY      | Outcome   | Access refused                  |

**Dimensions** (which check):

| Dimension  | Check          | Notes                  |
|------------|----------------|------------------------|
| IDENTITY   | Authentication | Who the caller is      |
| PERMISSION | Authorization  | What the caller may do |

**Pattern**: Acquisition's request-and-decision lexemes applied to access. DENY × IDENTITY is a bad
credential; DENY × PERMISSION is a known caller without the right.

**Translation to Status**: GRANT → STABLE, DENY → DEGRADED; ATTEMPT and CHALLENGE abstain. A
Scorecard weighs recent votes most, so the guard reads degraded once refusals outweigh grants among
its recent decisions, as in an attack, an identity store outage, or after a bad policy change. With
little history the reading is TENTATIVE, and a single refusal can tip it. No `OPERATION` map: a guard serves many requests at once, and a bracket
reading one trace per subject would take a second `ATTEMPT` for an abandoned episode. Sessions and
tokens a guard issues are leases; report them through Leases.

**When the identity store fails**: the guard reports what it decided, DENY when it fails closed and
GRANT when it fails open. The outage is reported on the store's own subject through Services or
Probes, which is where an observer tells an outage from a wave of bad credentials.

**Use cases**: Login failure rates, multi-factor challenge rates, authorization denials by policy
...
(SPACE/FLOW/LINK/TIME). Universal vocabulary that works across all system types regardless of
whether the underlying metric polarity is "high is good" or "low is good". Dimensions represent the
four fundamental constraints of any computational system.
...
affected), Statuses provides overall behavioral assessment (how is the system behaving). Example:
ALARM × FLOW + ALARM × SPACE → Status emits DEGRADED × CONFIRMED.
...
### Outcomes API (3 signs, no dimensions)

| Sign    | Category | Notes                                                                       |
|---------|----------|-----------------------------------------------------------------------------|
| SUCCESS | Outcome  | Operation succeeded                                                         |
| FAIL    | Outcome  | Operation failed                                                            |
| UNKNOWN | Outcome  | Verdict-bearing but indeterminate (distinct from a `null` non-verdict sign) |

**Pattern**: Minimal verdict vocabulary. No source context carried - if you need to know *why*
something failed, subscribe to the domain API. Outcomes answers one question: did it work, fail, or
remain indeterminate? `UNKNOWN` covers genuinely unsettled results (a conflict that may retry or
abort, an ambiguous disconnect) without forcing a polarity.

**Key Insight**: Semiotic ascent trades specificity for universality. The cause is lost; the verdict
remains. This enables universal aggregation across diverse domain vocabularies.
...
### Operations API (3 signs, no dimensions)

| Sign    | Category  | Notes                                              |
|---------|-----------|----------------------------------------------------|
| BEGIN   | Operation | Episode opening                                    |
| ADVANCE | Operation | Mid-episode step — neither opens nor closes a span |
| END     | Operation | Episode closing                                    |

**Pattern**: Minimal episode-structure vocabulary. Every episode opens, may progress, and closes.
Combined with a quality reading (`STATUS`, or `Outcomes` for a success/fail tally), you get the
complete picture: BEGIN → ADVANCE → END.

**Key Insight**: Universal span tracking. If you need to know *what kind* of action, subscribe to
the domain API. Operations answers: when did it start and stop?

**Use cases**: Cross-vocabulary action span tracking, duration measurement, nesting depth analysis,
...
**Pattern**: Statistical process control vocabulary based on Nelson's 8 Rules. Abstracts pattern
detection into universal categories.
...
**Use cases**: Statistical process control (SPC), anomaly detection, trend analysis, pattern-based
...
**Pattern**: Collective assessment vocabulary - generic over any Sign type (like Cycles). The Sign
comes from the source API being surveyed (typically Statuses.Sign, but could be any domain sign).
...
**Dimension Type**: Spectrum (ordered: DIVIDED → MAJORITY → UNANIMOUS). This enables reasoning about
agreement strengthening or weakening over time.
...
TENTATIVE confidence, mixed signals across cluster → may indicate network partition or inconsistent
state.
...
**Pattern**: Meta-level sign repetition pattern observation. Cycles is generic over any Sign type -
the Sign comes from the source API being observed. Cycles defines only the Dimension, describing
...
**Use cases**: Pattern recognition, state machine analysis, repetition detection, sign stream
analytics, cycle detection in observability data

**Translation to Status**: Long REPEAT sequences → STABLE, rapid RETURN patterns → ERRATIC, many
SINGLE without REPEAT → DIVERGING
...
**Processes**: SPAWN → START → STOP/FAIL/CRASH **Tasks**: SUBMIT → START → COMPLETE/FAIL
...
The counts tally the category column of every API table above, one row per sign slot, so they sum
to the registry's 237.

| Category               | Count | % of Total | APIs                                                                                                                                                                                                                         |
|------------------------|-------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Operation**          | 69    | 29%        | Actors, Atomics, Breakers, Caches, Counters, Exchanges, Gauges, Guards, Latches, Leases, Locks, Messages, Operations, Pipelines, Pools, Probes, Processes, Queues, Resources, Routers, Services, Stacks, Tasks, Transactions |
| **Outcome**            | 48    | 20%        | Atomics, Breakers, Caches, Changes, Evals, Flows, Guards, Latches, Leases, Locks, Members, Messages, Outcomes, Probes, Processes, Resources, Services, Tasks, Timers, Transactions                                           |
| **State Transition**   | 20    | 8%         | Breakers, Changes, Members, Processes, Services, Statuses, Tasks, Transactions                                                                                                                                               |
| **Error**              | 16    | 7%         | Latches, Leases, Locks, Members, Messages, Pipelines, Processes, Resources, Services, Tasks, Transactions                                                                                                                    |
| **Flow Control**       | 13    | 5%         | Pipelines, Processes, Services, Tasks                                                                                                                                                                                        |
| **Condition**          | 12    | 5%         | Counters, Gauges, Logs, Members, Pipelines, Queues, Stacks, Transactions                                                                                                                                                     |
| **Pattern**            | 5     | 2%         | Trends (STABLE, DRIFT, SPIKE, CYCLE, CHAOS)                                                                                                                                                                                  |
...
| **Progress**           | 4     | 2%         | Changes, Pipelines, Tasks                                                                                                                                                                                                    |
...
| **Anomaly**            | 2     | 1%         | Routers (CORRUPT, REORDER)                                                                                                                                                                                                   |
| **Contention**         | 2     | 1%         | Atomics (FAIL), Locks (CONTEST)                                                                                                                                                                                              |
...
| **Condition/Error**    | 1     | <1%        | Logs (SEVERE)                                                                                                                                                                                                                |
| **Emergency Response** | 1     | <1%        | Valves (DROP)                                                                                                                                                                                                                |
| **Recovery Response**  | 1     | <1%        | Valves (DRAIN)                                                                                                                                                                                                               |

**Inventory**: 237 sign slots drawn from 161 distinct lexemes, across 40 registry entries — 32
registered domain vocabularies, six universal vocabularies, and two vocabulary templates. A *slot*
is one sign in one vocabulary; the same lexeme appearing in two vocabularies is two slots and two
distinct signs. See `serventis-api-spec/REGISTRY.md` for the authoritative inventory.

### Universal SDK APIs

The `sdk/` package contains the universal vocabularies that abstract over domain-specific signs,
plus the two **vocabulary templates**, which fix a dimension set and take their sign set from the
caller. `Cycles` lives in `sdk/meta` rather than `sdk`; the rest are in `sdk`.
...
| Outcomes   | SUCCESS, FAIL, UNKNOWN                                            | —                              | Verdict - did it work, fail, or stay unknown? |
| Operations | BEGIN, ADVANCE, END                                               | —                              | Episode brackets - when did it run?           |
| Trends     | STABLE, DRIFT, SPIKE, CYCLE, CHAOS                                | —                              | Pattern detection - how is it behaving?       |
| Systems    | NORMAL, LIMIT, ALARM, FAULT                                       | SPACE, FLOW, LINK, TIME        | Constraint class - which limit is implicated? |
| Surveys *  | (supplied at materialization)                                     | DIVIDED, MAJORITY, UNANIMOUS   | Collective agreement - how much do we agree?  |
| Cycles *   | (supplied at materialization)                                     | SINGLE, REPEAT, RETURN         | Repetition patterns - how are signs cycling?  |

`*` vocabulary template, not a vocabulary: it publishes no `SIGNS`.

These complement domain APIs by providing cross-vocabulary aggregation points.

**The universal set** — six vocabularies and two templates (ordered by semiotic hierarchy):
...
3. **Operations**: BEGIN/ADVANCE/END (when did it run?)
4. **Outcomes**: SUCCESS/FAIL/UNKNOWN (did it work?)
5. **Trends**: STABLE/DRIFT/SPIKE/CYCLE/CHAOS (how is it behaving?)
6. **Systems**: NORMAL/LIMIT/ALARM/FAULT × SPACE/FLOW/LINK/TIME (which constraint is implicated?)
7. **Surveys** *(template)*: (Sign × DIVIDED/MAJORITY/UNANIMOUS) (how much do we agree?)
8. **Cycles** *(template)*: (Sign × SINGLE/REPEAT/RETURN) (how are signs cycling?)

**Hierarchy**: Statuses and Situations form the **semiotic ascent layer**, translating domain signs
upward into universal operational vocabulary. The remaining six provide **meta-level observation**
capabilities.
...
Signs of a *primary* vocabulary represent what the implementer can **directly observe**. Six
registered vocabularies are *derived* and exempt by design — `Sensors`, `Logs`, `Evals`, `Timers`,
`Routers`, and `Pipelines` — as are the universal assessment languages. A vocabulary is derived when
selecting any component of an observation needs a comparison, a measurement, or a memory of what
came before; `serventis-api-spec/REGISTRY.md` records the classification per vocabulary, and
`SPEC.md` §8.1.1 defines the test.
...
- ✅ Application code knows: an error occurred (though selecting `Logs.SEVERE` over `WARNING` is a
  judgment, which is why `Logs` is a *derived* vocabulary)
