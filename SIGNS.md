...
outcomes. As of Serventis 3.0 this binary is executable: each outcome-oriented domain publishes a
`public static final SignMap<Sign, Serventis.Kind> KIND` (total, exhaustive — no `default`), the
structural twin of its `STATUS` map. `KIND` is *grammatical* (act vs result) and deliberately
independent of the *evaluative* `STATUS` reading. It describes a tendency in how a sign carries
evidence; it does **not** select which interpretation a sign feeds — operations may carry per-sign
status readings and outcomes may be structurally significant. See `Serventis.Kind` and "The Kind of
a Sign" in `SERVENTIS.md`.
...
**Translation to Status** (pattern-derived, not a canonical map — Actors publishes only `SIGNS`):
...
**Translation to Status** (pattern-derived, not a canonical map — Pipelines publishes only `SIGNS`):
high LAG → DEGRADED, sustained OVERFLOW → DEGRADED, many SKIP → DEFECTIVE (subscriber inferences
over sign patterns)
...
**Translation to Status** (pattern-derived, not a canonical map — Gauges publishes only `SIGNS`):
...
**Translation to Status**: High PUSH without POP → DIVERGING, many OVERFLOW → DEGRADED, many
UNDERFLOW → DEGRADED, rapid PUSH/POP → ERRATIC
...
**Translation to Status** (pattern-derived, not a canonical map — Exchanges publishes only `SIGNS`
and `DIMENSIONS`): Many CONTRACT without TRANSFER → DEGRADED, high TRANSFER rate → STABLE,
asymmetric CONTRACT (one side only) → DIVERGING (subscriber inferences over sign patterns)
...
| **Outcome**            | 40    | 19%        | Services, Tasks, Resources, Transactions, Flows, Timers, Caches, Leases, Probes, Agents, Evals                          |
...
**Inventory**: 213 sign slots drawn from 148 distinct lexemes, across 36 registry entries — 28
