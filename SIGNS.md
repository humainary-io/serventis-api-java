...
DEFECTIVE, sustained MEET → STABLE
...
**Translation to Status**: High FAIL/ATTEMPT ratio → DEGRADED, frequent PARK → DEGRADED,
EXHAUST signals → DEFECTIVE, low FAIL rate → STABLE
...
**Translation to Status**: High MISS rate → DEGRADED, many EVICT → DEGRADED,
many EXPIRE → DIVERGING
...
**Translation to Status**: High LAG → DEGRADED, OVERFLOW → DEGRADED, many SKIP → DEFECTIVE
...
**Translation to Status**: Many SEVERE → DEFECTIVE, many WARNING → DIVERGING, high DEBUG →
ERRATIC
...
**Translation to Status**: OVERFLOW → DEGRADED, UNDERFLOW → DEGRADED, high RESET rate → ERRATIC
...
**Translation to Status**: High DENY → DEGRADED, EXPAND activity → CONVERGING, CONTRACT + DENY →
DIVERGING, DROP → DEGRADED, DRAIN → CONVERGING
...
**Translation to Status**: Many EXPIRE without RENEW → DIVERGING, high DENY → DEGRADED,
...
**Translation to Status**: Many CONTRACT without TRANSFER → DEGRADED, high TRANSFER rate →
STABLE, asymmetric CONTRACT (one side only) → DIVERGING
...
**Translation to Status**: Sustained ABOVE × THRESHOLD → DEGRADED, ABOVE × BASELINE +
BELOW × THRESHOLD → DIVERGING, BELOW × TARGET → STABLE, oscillating around BASELINE →
STABLE
