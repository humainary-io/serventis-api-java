...
An **observation** is the unit of semiotic expression in Serventis—a **sign**, optionally
...
principal sign, and a qualified observation additionally exposes its dimension. A consumer that
needs only the classification operates over observations and obtains the sign uniformly—for an
atomic observation the sign is the observation itself, and for a qualified observation it is the
...
Every sign carries a **kind** within its set: it reports either an **operation**, an act the
producer performed, or an **outcome**, a result it observed. The kind is a
property of the sign within its set—stable and enumerable like the sign itself—and it describes how
the sign tends to carry evidence. An outcome is usually self-contained: a single occurrence licenses
a reading. An operation usually is not; its significance lies in the sequence it participates in—a
request that never reaches its outcome, a suspension with no resumption. That tendency is why
translation is partial: most operations have no reading to give on their own.

The kind does not *route* a sign to an ascent path, and must not be used to. Either kind may be read
individually or read for its position, and both cases are common: a lease `REVOKE` is an operation
carrying a `DEFECTIVE` status reading, while a lock `GRANT` is an outcome that merely advances an
episode. The translation tally reads whatever ballot it is given without consulting the kind, and
the sequencing operators read an episode-role classification rather than the kind. The two ascent
paths are distinguished by what supplies their evidence—the observation itself, or its position
among others—not by which kind of sign is being read.
...
operation. So may an act whose occurrence is itself the success, such as an enqueue the queue
accepted. Classification asks what the sign is; translation asks what status it indicates; the two
...
In the Java projection the kind is the `Kind` enum (`OPERATION`, `OUTCOME`), and every registered
domain vocabulary publishes it as a total `SignMap<Sign, Kind>`, the structural twin of the
...
judgment enters the trace as ordinary sequence shape; it is never hidden inside the sequencing
operator.
