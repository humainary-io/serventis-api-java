...
* **Signal**: A record composing Sign × Dimension into a single observable event. Each
  two-dimensional
...
* **opt/exec** (`io.humainary.serventis.opt.exec`): Execution — Tasks, Services, Transactions,
  Processes, Timers, Changes. Work units, service interactions, transactional coordination, and
  changes made to a subject.

* **opt/data** (`io.humainary.serventis.opt.data`): Data structures — Queues, Stacks, Caches,
  Pipelines, Messages. Bounded container and data flow operations, and message delivery.
...
* **opt/flow** (`io.humainary.serventis.opt.flow`): Flow control — Flows, Routers, Breakers, Valves,
  Guards. Routing, circuit breaking, backpressure, and access decisions.
...
* **opt/role** (`io.humainary.serventis.opt.role`): Coordination — Agents, Actors, Members.
  Promise-based and conversational interaction models, and group membership.
