...
### 8.2 Operations — the minimal open/close bracket (no `STATUS`; the Sequencer is the only reader)
...
The machine has no transition for a refused request, because Actors has no sign for one. `DENY`
means disagreeing with or correcting a proposition, so reading it as a refusal would misread every
disagreement raised mid-exchange. A refused request therefore leaves the walk awaiting a
`PROMISE`.
...
|----------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------|
| Locks, Resources, Leases, Tasks, Processes, Probes, Agents, Changes        | **Breakers** — persistent cycle (recognize its *flap* as resetting pairs, §3)                                                         |
| Operations (generic open/close), Actors, Pipelines (escalation trajectory) | **Services, Exchanges, Messages** — dimensional, opener varies by role (§6.3); Services is **excluded** from the generic bracket (§9) |
| Atomics (`SPIN→YIELD→BACKOFF→PARK` deepening trajectory)                   | **Pools** — counting, not pairing (§6.2)                                                                                              |
|                                                                            | **Members** — a standing state, not an episode                                                                                        |
|                                                                            | **Guards** — many concurrent requests share one subject, so consecutive `ATTEMPT`s are normal                                         |
...
A Sequencer emits **status**, not a *Situation*. Elevating status → situation needs evidence a
recognizer of sequence shape does not hold, so no Sequencer produces one.

Duration is the most familiar such evidence — "elevated errors for three hours, unchanging →
CRITICAL/CONSTANT" reads a dwell window — but it is an example, not the definition. Confidence,
correlation across subjects, the consequence of acting, and deployment policy can each determine a
situation reading instead, and the specification deliberately constrains none of them.

```text
domain signs ──Sequencer (structure)──► Statuses.Sign ──situation assessor──► Situations.Sign
```

The Sequencer's contribution to situation is **indirect**: clean, structurally-grounded status input
makes whatever downstream elevation you choose trustworthy. Reach for a situation assessor — not the
