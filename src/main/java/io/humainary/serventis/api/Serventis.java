...
public interface Serventis
  ...
  /// degraded-mode recourse — stay [#OPERATION] here while `STATUS` still reads them. So do a few
  /// acts whose occurrence is itself the success, such as an enqueue the queue accepted.
  ///
  /// Every registered domain vocabulary materializes the classification as a `public static final
  /// SignMap<Sign, Kind> KIND` — total (none abstains) and exhaustive (a new sign is a compile error
  /// until classified), the structural twin of the partial, outcome-reading `STATUS` map. A
  /// vocabulary may classify every sign one way: `Logs` has only outcomes, `Actors` only operations.
  /// `Kind` is not (yet) carried on [Sign] itself; the per-domain map is its home until a generic,
  /// Sign-agnostic operator needs to read it off an arbitrary sign.
