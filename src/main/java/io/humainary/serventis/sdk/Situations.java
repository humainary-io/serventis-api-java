...
public final class Situations
  ...
  /// situation signal instances and caller-derived signal maps.
  ...
  /// Creates a Situation instrument wrapping the specified pipe.
  ///
  /// Example usage:
  /// ```java
  /// final var cortex = Substrates.cortex();
  /// var situations = circuit.conduit(Signal.class).pool(Situations::of);
  /// var situation = situations.get(cortex.name("db.pool"));
  /// ```
  ///
  /// @param pipe the pipe from which to create the situation
  /// @return a new Situation instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Situation instruments from a conduit.
  ///
  ...
  /// Example usage:
  /// ```java
  /// Pool<Situation> situations = Situations.pool(conduit);
  /// var situation = situations.get(name);
  /// ```
  ///
  /// @param conduit the conduit providing signal pipes
  /// @return a pool that creates Situation instruments
  /// @throws NullPointerException if the conduit parameter is `null`

  @SpecRef ( {"6.4", "substrates:10.1"} )
  @New
  @NotNull
  public static Pool < Situation > pool (
