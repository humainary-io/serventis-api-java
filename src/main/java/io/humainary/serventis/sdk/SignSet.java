...
/// alongside the default Java-projection interpretations (`STATUS`, `KIND`) where the domain ships them. A
...
/// shared freely across threads and circuits without synchronization. Maps created from it have
/// immutable mapping structures; mapped values are retained by reference and their thread safety
...
public final class SignSet <
  ...
  /// Creates a signal set: the `Sign × Dimension` product of this sign set with `dimensions`, each cell
  ...
  /// @param dimensions the dimension set (a domain's published `DIMENSIONS`)
  ...
  /// @throws NullPointerException if `dimensions` or `factory` is `null`

  @New
  @NotNull
  public < D extends Enum < D > & Dimension, T extends Record & Signal < S, D > >
  SignalSet < S, D, T > signals (
    @NotNull final SymbolSet < D > dimensions,
    ...
    return
      new SignalSet <> (
        signs.symbols,
        requireNonNull ( dimensions ).symbols,
        factory
      );
