...
/// ```
///
/// - **Domain APIs** (Resources, Tasks, Services, etc.): Publish a `STATUS` map an application may
///   read to translate their signs upward
/// - **Probes API**: Provides raw observation data that feeds into status assessment
/// - **Situations API**: Reports situational assessments supplied by an application assessor
/// - **Sensors API**: Positional measurements an application may translate into status conditions
///
/// These are assessment stages an application may compose, not built-in translation. A Status
/// instrument emits the reading its caller supplies; it does not calculate one. [Scorecards]
/// is the supplied operator for folding domain signs into statuses; there is no built-in
/// status-to-situation operator.
///
...
public final class Statuses
  ...
  /// status signal instances and caller-derived signal maps.
  ...
  /// Creates a Status instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the status
  /// @return a new Status instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Status instruments from a conduit.
  ///
