...
/// ## Contract scope
///
/// This type is a Java helper. Its interpretation and execution rules are not portable
/// Serventis conformance requirements. Vocabulary meanings and instrument emission semantics
/// remain governed by the specification; published property maps provide this projection's readings.
///
...
public final class Sequencers
  ...
  /// @param <S>    the sign enum type the machine reads
  /// @param status the status reading to emit, or `null` for silence
  /// @param next   the next state map, or `null` to reset to the root
  /// @return a transition to `next`

  @New
  @NotNull
  public static < S extends Enum < S > & Sign > Transition < S > emit (
    final Statuses.Sign status,
    final SignMap < S, Transition < S > > next
  ) {
  ...
  ///
  /// `root` is the root state map — the initial, usually idle, state. If the current state returns
  ...
  public static < S extends Enum < S > & Sign > Flow < S, Statuses.Sign > flow (
    @NotNull final SignMap < S, Transition < S > > root
  ) {
  ...
  /// trajectory for a domain that supplies suitable maps, without a hand-written machine:
  /// `Sequencers.flow ( Locks.OPERATION, Locks.STATUS )` works as well as
  /// `Sequencers.flow ( Transactions.OPERATION, Transactions.STATUS )`.
  ///
  /// Use one correlated, non-overlapping episode stream per attachment. The recognizer does not
  /// track concurrent or nested episodes, correlate subjects, or preserve dimensions. Separate
  /// independent streams upstream. ACQUIRE → GRANT → RELEASE is one lock episode; two overlapping
  /// ACQUIRE signs share one state and cannot represent two independent acquisitions here.
  ///
  /// `OPERATION` supplies the episode *structure* — which signs open, advance, and close a span;
  /// `STATUS` supplies the per-sign *quality*, the per-sign health reading supplied by the map. The trajectory:
  ///
  ...
  ///
  /// **Contract on `status`.** Pass a canonical per-sign `STATUS` map. Such a map draws on the
  /// canonical *level* subset — [Statuses.Sign#STABLE], [Statuses.Sign#DEGRADED],
  /// [Statuses.Sign#DEFECTIVE] — or `null` (abstains). `Statuses` as a whole is not ordered and is not
  /// a lattice; this operator defines no ordering over it beyond the severity fold described below.
  /// The operator never echoes the input verbatim — it emits a *trajectory* reading. The
  ...
  /// @param status    the domain's canonical `STATUS` map (sign → [Statuses.Sign] level subset, partial)
  ...
  public static final class Transition <
    ...
    private final SignMap < S, Transition < S > > next;
