...
/// [Statuses.Signal], so a scored assessment is indistinguishable from a hand-emitted status and can
/// be consumed anywhere a status is. Ascending further into [Situations] is a separate step: no
/// status-to-situation operator is provided, since that reading depends on dwell, cost, and
/// correlation that the stream does not carry, so a situation assessor must be supplied and wired
/// up by the consumer.
///
...
/// *continuous* — a cumulative tally asymptotes and stops tracking recent behavior.
///
/// The winner's confidence is banded into the [Statuses.Dimension] spectrum, honoring its
...
/// so a vote reduces to a banding comparison plus one array lookup, allocating nothing of its own.
/// The per-subject tally holds a small `double[]` and runs single-threaded on the circuit, so it is
/// safe without synchronization. End-to-end emission allocation and latency depend on the
/// underlying Substrates provider and pipeline; measure `ScorecardOps` in `perfkit` for figures.
///
/// ## Contract scope
///
...
public final class Scorecards
  ...
  ///
  /// This is **not** an allocation-free path, unlike [#flow]. The per-call bucket accumulator is
  /// passed through `Window.fold` — an opaque cross-module call — so escape analysis does not
  /// eliminate it, and the substrate allocates a window view per emission besides. Measure
  /// `ScorecardOps` in `perfkit` rather than assuming a figure.
  ///
  ...
  private static final class Tally {
    ...
    /// Records one vote. A `null` target abstains: state is left untouched and the next
