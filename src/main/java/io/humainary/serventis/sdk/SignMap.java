...
///
/// The mapping structure is immutable and safe for concurrent lookup. Values are retained by
/// reference; they are not copied or frozen, and their mutability and thread safety remain the
/// caller's responsibility. Mapping functions run synchronously during construction. A thrown
/// failure aborts construction and propagates to the caller; it is not an emission-time fault.
///
/// ## Contract scope
///
...
///
/// Where a domain calls its published `STATUS`, `KIND`, `OPERATION` or `OUTCOME` map **canonical**,
/// the word distinguishes the map the domain ships from one a caller derives. It does not mean the
/// reading is required of another projection.
///
/// ## Type Parameters
///
