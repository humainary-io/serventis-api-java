...
/// - **Statuses API**: Reports operational assessments supplied by the application
/// - **Situations API**: Reports urgency assessments supplied by the application
///
/// The arrows describe possible application-supplied assessment stages, not built-in translation.
/// Instruments emit supplied readings; they do not calculate them. BELOW identifies position below
/// a reference, without encoding proximity, direction of movement, or urgency.
///
...
///   - `BELOW × THRESHOLD` (below the limit)
///
...
public final class Sensors
  ...
  /// sensor signal instances and caller-derived signal maps.
  ...
  /// Canonical sign-to-kind classification for sensors — all three positions are [Kind#OUTCOME]: each
  /// is a reading of a measured value against a setpoint, not an act performed toward a result.
  /// Sensors has no operations of its own, and the setpoint dimension does not change the kind.
  /// Exhaustive without a `default`. See [Kind].

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case BELOW, NOMINAL, ABOVE -> OUTCOME;
      }
    );

  private Sensors () { }

  /// Creates a Sensor instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe provided by the conduit for emitting setpoint signals
  /// @return a new Sensor instrument for the specified pipe
  /// @throws NullPointerException if the pipe parameter is `null`

  @SpecRef ( "6.4" )
  @New
  @NotNull
  public static Sensor of (
  ...
  /// Returns a pool that creates cached Sensor instruments from a conduit.
  ///
