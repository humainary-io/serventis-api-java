...
public final class Queues
  ...
  /// Canonical sign-to-status translation for queues — the default *immediate interpretant* of the
  /// upward ascent (compose to override; see [SignMap]). An accepted enqueue reads healthy, the way
  /// a resource grant does; an enqueue refused at capacity reads degraded. `DEQUEUE` abstains, and so
  /// does `UNDERFLOW`: a consumer finding the queue empty is normal flow, not a fault. The
  /// enqueue/overflow *ratio* emerges from the Scorecard's plurality.
  ///
  ...
  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        ...
        case DEQUEUE, UNDERFLOW -> null;
      }
    );

  /// Canonical sign-to-kind classification for queues — each [Sign] tagged [Kind#OPERATION] or
  /// [Kind#OUTCOME]: enqueue and dequeue are operations; the overflow/underflow boundary results are
  /// outcomes. `ENQUEUE` is an `OPERATION` though [#STATUS] reads it `STABLE` (see [Kind]).
  ...
  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        ...
        case ENQUEUE, DEQUEUE -> OPERATION;
      }
    );

  private Queues () { }

  /// Creates a Queue instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the queue
  /// @return a new Queue instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Queue instruments from a conduit.
  ///
