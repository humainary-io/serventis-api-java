...
///
/// - **Locks API**: Locks ARE resources - vocabulary aligned (ATTEMPT, ACQUIRE, GRANT, DENY, TIMEOUT, RELEASE)
...
/// - **Situations API**: Resource pressure patterns can elevate situations from NORMAL to WARNING
///
...
@SpecRef ( {"8.2", "registry:resources"} )
public final class Resources
  ...
  /// The canonical sign-to-status translation for resources — the *immediate interpretant* of this
  /// vocabulary's upward ascent. The outcome signs carry a status reading; the operation signs
  /// (`ATTEMPT`, `ACQUIRE`, `RELEASE`) abstain, having no direct upward path. Consumers wrap it with
  /// `Scorecards.flow ( STATUS )` for the default reading, or compose to override for a
  /// context-specific *dynamic interpretant* (see [SignMap]).
  ///
  ...
  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        ...
        case ATTEMPT, ACQUIRE, RELEASE -> null;
      }
    );

  /// Canonical sign-to-kind classification for resources — each [Sign] tagged [Kind#OPERATION] (an
  /// act performed) or [Kind#OUTCOME] (a result observed): the attempt/acquire/release request
  /// actions are operations; the grant/deny/timeout verdicts are outcomes. Exhaustive without a
  /// `default`. See [Kind].

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case GRANT, DENY, TIMEOUT -> Kind.OUTCOME;
        case ATTEMPT, ACQUIRE, RELEASE -> Kind.OPERATION;
      }
    );

  /// Sign-to-[Operations] classification — the **episode structure** of each sign (see `SEQUENCERS.md`): a
  /// try-attempt or a blocking acquire opens the span ([Operations.Sign#BEGIN]); a grant holds it ([Operations.Sign#ADVANCE]);
  /// a denial, timeout, or release ends it ([Operations.Sign#END]). Total and exhaustive without a `default`.

  public static final SignMap < Sign, Operations.Sign > OPERATION =
    SIGNS.map (
      sign -> switch ( sign ) {
        case ATTEMPT, ACQUIRE -> Operations.Sign.BEGIN;
        case GRANT -> Operations.Sign.ADVANCE;
        case DENY, TIMEOUT, RELEASE -> Operations.Sign.END;
      }
    );

  /// Sign-to-[Outcomes] translation — the *outcome* half of the grammatical interlingua: a grant reads
  /// [Outcomes.Sign#SUCCESS]; a denial or timeout reads [Outcomes.Sign#FAIL]. The operations abstain.
  /// Exhaustive without a `default`, consistent with [#KIND].

  public static final SignMap < Sign, Outcomes.Sign > OUTCOME =
    SIGNS.map (
      sign -> switch ( sign ) {
        case GRANT -> SUCCESS;
        case DENY, TIMEOUT -> FAIL;
        case ATTEMPT, ACQUIRE, RELEASE -> null;
      }
    );
  ...
  /// Creates a Resource instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the resource
  /// @return a new Resource instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Resource instruments from a conduit.
  ///
  ...
  @SpecRef ( {"4.2", "registry:resources"} )
  public enum Sign
