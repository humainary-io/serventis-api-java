...
public final class Leases
  ...
  /// lease signal instances and caller-derived signal maps.
  ...
  /// The canonical sign-to-status translation for leases — the *immediate interpretant* of this
  /// vocabulary's upward ascent (sign-keyed; the `LESSOR`/`LESSEE` perspective does not change the
  /// reading). Grants and extensions read healthy; denials and timeouts read degraded, a forced
  /// revocation reads defective (aligned with the `REVOKE → DEFECTIVE` classification in `SIGNS.md`);
  /// the request, renewal, release, and monitoring operations abstain. Compose to override (see
  /// [SignMap]); the instrument emits `Signal`, so as a Scorecards ballot project the sign first —
  /// `Scorecards.flow ( STATUS.compose ( Signal::sign ) )`.
  ///
  /// The switch is exhaustive without a `default`: adding a [Sign] becomes a compile error here until
  /// its status reading is decided.

  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        case GRANT, EXTEND -> STABLE;
        case DENY, EXPIRE -> DEGRADED;
        case REVOKE -> DEFECTIVE;
        case ACQUIRE, RENEW, RELEASE, PROBE -> null;
      }
    );

  /// Canonical sign-to-kind classification for leases — each [Sign] tagged [Kind#OPERATION] or
  /// [Kind#OUTCOME]: acquire/renew/release/probe and the authority's `REVOKE` are operations; the
  /// grant/deny/extend/expire verdicts are outcomes. `REVOKE` is an `OPERATION` though [#STATUS]
  /// reads it `DEFECTIVE` — act/result is independent of the status reading (see [Kind]). Exhaustive
  /// without a `default`.

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case GRANT, DENY, EXTEND, EXPIRE -> Kind.OUTCOME;
        case ACQUIRE, RENEW, RELEASE,
             REVOKE, PROBE -> Kind.OPERATION;
      }
    );

  /// Sign-to-[Operations] classification — the **episode structure** of each sign (see `SEQUENCERS.md`): the
  /// acquire opens the lease span ([Operations.Sign#BEGIN]); a grant, an extension, a renewal, and a liveness probe
  /// progress within it ([Operations.Sign#ADVANCE]); a denial, expiry, release, or revoke ends it ([Operations.Sign#END]).
  /// Total and exhaustive without a `default`.

  public static final SignMap < Sign, Operations.Sign > OPERATION =
    SIGNS.map (
      sign -> switch ( sign ) {
        case ACQUIRE -> Operations.Sign.BEGIN;
        case GRANT, EXTEND, RENEW, PROBE -> Operations.Sign.ADVANCE;
        case DENY, EXPIRE, RELEASE, REVOKE -> Operations.Sign.END;
      }
    );

  /// Sign-to-[Outcomes] translation — the *outcome* half of the grammatical interlingua: a grant and
  /// an extension read [Outcomes.Sign#SUCCESS]; a denial or expiry reads [Outcomes.Sign#FAIL]. The
  ...
  public static final SignMap < Sign, Outcomes.Sign > OUTCOME =
    SIGNS.map (
      sign -> switch ( sign ) {
        case GRANT, EXTEND -> SUCCESS;
        case DENY, EXPIRE -> FAIL;
        case ACQUIRE, RENEW, RELEASE,
             REVOKE, PROBE -> null;
      }
    );
  ...
  /// Creates a Lease instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the lease
  /// @return a new Lease instrument for the specified pipe
  /// @throws NullPointerException if the pipe parameter is `null`

  @SpecRef ( "6.4" )
  @New
  @NotNull
  public static Lease of (
  ...
  /// Returns a pool that creates cached Lease instruments from a conduit.
  ///
  ...
  ///
  /// @param sign      the lease lifecycle event
  /// @param dimension the lease-side perspective for the event
