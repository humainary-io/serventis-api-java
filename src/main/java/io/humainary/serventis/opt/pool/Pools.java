...
/// - **Statuses API**: Pool patterns (high BORROW) inform Status conditions (DEGRADED)
...
/// |---------------------------|-------------|----------------------------------------|
/// | High BORROW, low RECLAIM  | DEGRADED    | High utilization, limited availability |
/// | BORROW without RECLAIM    | DEFECTIVE   | Resources not returned (leak)          |
/// | Rapid EXPAND/CONTRACT     | ERRATIC     | Capacity oscillating                   |
/// | EXPAND activity           | CONVERGING  | Pool adapting to demand                |
/// | High BORROW rate          | DIVERGING   | Pool heavily utilized                  |
/// | CONTRACT to minimum       | STABLE      | Pool at minimum capacity               |
/// | EXPAND stalls under BORROW| DEGRADED    | Pressure persists, pool stops growing  |
///
/// ### Status → Situation Assessment
///
/// | Status Pattern       | Situation   | Example                                    |
/// |----------------------|-------------|--------------------------------------------|
/// | DEGRADED (sustained) | WARNING     | Pool too small for demand                  |
/// | DEFECTIVE (detected) | CRITICAL    | Resource leak in application               |
/// | ERRATIC (oscillating)| WARNING     | Pool sizing algorithm hunting              |
/// | CONVERGING (effective)| NORMAL     | Pool adapting to load successfully         |
///
...
public final class Pools
  ...
  /// Canonical sign-to-kind classification for pools — all four signs are [Kind#OPERATION]:
  /// expansion, contraction, borrowing, and reclamation are acts the pool performs. Pools has no
  /// outcomes of its own. Exhaustive without a `default`. See [Kind].

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case EXPAND, CONTRACT, BORROW, RECLAIM -> OPERATION;
      }
    );

  private Pools () { }

  /// Creates a Pool instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the pool
  /// @return a new Pool instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Pool instruments from a conduit.
  ///
