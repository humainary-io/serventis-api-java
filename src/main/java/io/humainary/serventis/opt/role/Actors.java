...
///
/// - **Statuses API**: Actors AFFIRM monitor conditions about system state
/// - **Counters, Gauges, Caches**: Actors REPORT observations from instruments
/// - **Resources API**: Actors REQUEST actions on system components
/// - Meta-observers observe dialogue patterns to detect coordination failures,
...
/// var human = circuit.conduit(Sign.class).pool(Actors::of).get(cortex.name("user.william"));
/// var ai = circuit.conduit(Sign.class).pool(Actors::of).get(cortex.name("assistant.claude"));
///
...
public final class Actors
  ...
  /// Canonical sign-to-kind classification for actors — all eleven speech acts are
  /// [Kind#OPERATION]: each is an act the speaker performs, and the utterance is the event. Actors
  /// has no outcomes of its own. Exhaustive without a `default`. See [Kind].

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case ASK, AFFIRM, EXPLAIN, REPORT,
             REQUEST, COMMAND, ACKNOWLEDGE, DENY,
             CLARIFY, PROMISE, DELIVER -> OPERATION;
      }
    );

  private Actors () { }

  /// Creates an Actor instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the actor
  /// @return a new Actor instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Actor instruments from a conduit.
  ///
  ...
  public enum Sign
    ...
    /// interpretive commitment. More objective than AFFIRM. Use for presenting
