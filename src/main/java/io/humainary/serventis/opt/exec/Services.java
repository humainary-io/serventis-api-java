...
/// retries, redirections, suspensions, and more. The role dimension (CALLER/CALLEE)
...
/// ## Role Dimension
///
/// Each signal has one role dimension, with two possible values:
///
...
///
/// ### Execution Lifecycle
/// - **START**: Work execution begins
/// - **STOP**: Work execution completes (regardless of outcome)
/// - **CALL**: Remote work request issued
///
/// ### Outcomes
...
///
/// ### Flow Control
/// - **DELAY**: Work postponed
/// - **SCHEDULE**: Work queued for future execution
/// - **SUSPEND**: Work paused, may resume
/// - **RESUME**: Suspended work restarted
///
/// ### Error Handling
/// - **RETRY**: Failed work being reattempted
/// - **RECOURSE**: Degraded mode activated
/// - **REDIRECT**: Work forwarded to alternative service
///
/// ### Rejection
/// - **REJECT**: Work declined (e.g., overload, policy)
/// - **DISCARD**: Work dropped (e.g., expired, invalid, or a call the caller canceled)
/// - **DISCONNECT**: Unable to reach service
///
/// ### Temporal
/// - **EXPIRE**: Work exceeded time budget
///
...
public final class Services
  ...
  /// service signal instances and caller-derived signal maps.
  ...
  /// Canonical sign-to-status translation for services — the default *immediate interpretant* of the
  /// upward ascent (sign-keyed; the CALLER/CALLEE perspective does not change the reading; compose to
  /// override, see [SignMap]). Success reads healthy; failure, disconnection, rejection, expiry, and
  /// degraded-mode recourse read degraded; the call/execution lifecycle and the
  /// retry/redirect/discard/delay/schedule/suspend/resume flow-control operations abstain. The
  ...
  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        case SUCCESS -> STABLE;
        case FAIL, DISCONNECT, REJECT,
             EXPIRE, RECOURSE -> DEGRADED;
        case START, STOP, CALL, REDIRECT, RETRY,
             DISCARD, DELAY, SCHEDULE,
             SUSPEND, RESUME -> null;
      }
    );

  /// Canonical sign-to-kind classification for services — every sign tagged [Kind#OPERATION] (an act
  /// the service performed) or [Kind#OUTCOME] (a result it observed); the CALLER/CALLEE perspective
  /// does not change the kind. The work lifecycle (START/STOP/CALL), the retry/redirect/recourse
  /// recovery moves, and the discard/delay/schedule/suspend/resume flow control are operations;
  /// success, failure, expiry, boundary rejection, and disconnection are outcomes.
  ///
  /// `RECOURSE` is an `OPERATION` even though [#STATUS] reads it `DEGRADED`: the act/result
  /// classification is deliberately independent of the status reading (see [Kind]).
  ///
  ...
  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case SUCCESS, FAIL, EXPIRE,
             REJECT, DISCONNECT -> Kind.OUTCOME;
        case START, STOP, CALL, RECOURSE, REDIRECT,
             RETRY, DISCARD, DELAY, SCHEDULE,
             SUSPEND, RESUME -> Kind.OPERATION;
      }
    );

  /// `Services` is **excluded from the `OPERATION`/`OUTCOME` interlingua** (and so from the generic
  /// bracket sequencer). It is dimensional in a way the single-span bracket cannot model: the opener
  /// flips by perspective (`CALL` for a caller, `START` for a callee), and an outbound call nests inside
  /// the current work — its own `SUCCESS`/`FAIL` would erase to a premature close. Translating its signs
  /// upward needs dimension awareness: split by `CALLER`/`CALLEE` first (see `SEQUENCERS.md` §6.3), then
  /// hand-write a per-perspective machine. `STATUS` and `KIND` (per-sign, structure-free) remain.
  ...
  /// Creates a Service instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the service
  /// @return a new Service instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Service instruments from a conduit.
  ///
  ...
  public enum Sign
    ...
    ///
    /// Either end may drop the work. From the CALLEE perspective, the service dropped work it had
    /// accepted. From the CALLER perspective, the service dropped its own outstanding call: it
    /// canceled the call or stopped waiting for the reply.
    ///
  ...
  ///
  /// A service is an instrument that emits signals.
  ///
  /// ## Usage
  ///
  ...
  public static final class Service
    ...
    /// Invokes the supplied function synchronously on the calling thread.
    ///
    /// Emits CALL before invocation, then SUCCESS after normal return or FAIL before
    /// rethrowing the original failure.
    /// Only the observations are queued through the pipe; the client code is not scheduled
    /// on the circuit and its failures are not isolated as receptor failures.
    ///
    /// @param dimension the role (CALLER or CALLEE)
    ...
    /// @throws NullPointerException if the dimension or function is `null`
    ...
    /// Invokes the supplied operation synchronously on the calling thread.
    ///
    /// Emits CALL before invocation, then SUCCESS after normal return or FAIL before
    /// rethrowing the original failure.
    /// Only the observations are queued through the pipe; the client code is not scheduled
    ...
    /// @throws NullPointerException if the dimension or operation is `null`
    ...
    /// Invokes the supplied function synchronously on the calling thread.
    ///
    /// Emits START before invocation, then SUCCESS after normal return or FAIL before
    /// rethrowing the original failure. `execute` also emits STOP in a finally block.
    /// Only the observations are queued through the pipe; the client code is not scheduled
    /// on the circuit and its failures are not isolated as receptor failures.
    ///
    /// @param dimension the role (CALLER or CALLEE)
