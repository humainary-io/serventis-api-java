...
/// - **Statuses API**: Process termination patterns inform conditions (CRASH → DEFECTIVE, FAIL/KILL → DEGRADED)
...
/// - Per-sign (the canonical `STATUS` map): clean STOP → STABLE; FAIL and KILL → DEGRADED (a kill is
///   often deliberate orchestration); an abnormal CRASH (segfault, panic, uncaught exception) →
///   DEFECTIVE
/// - Trend readings are the sequencer's, not per-sign: RESTART pattern → DIVERGING (supervision
///   active), crash loop (rapid CRASH/RESTART) → DOWN (failing to recover)
///
/// ### Process → Situation Assessment
/// - Crash loop → CRITICAL situation (service unavailable)
/// - Many KILL signals → WARNING situation (resource exhaustion, OOM)
/// - CRASH during transaction → CRITICAL situation (potential data corruption)
...
@SpecRef ( {"8.2", "registry:processes"} )
public final class Processes
  ...
  /// Canonical sign-to-status translation for processes — the default *immediate interpretant* of the
  /// upward ascent (compose to override; see [SignMap]). A clean stop reads healthy; a failed exit and
  /// a force-kill read degraded (a kill is often deliberate orchestration); an abnormal crash
  /// (segfault, panic, uncaught exception) reads defective; the spawn/start/restart lifecycle and
  /// suspend/resume abstain. Trend readings the prose lists (restart pattern → diverging, crash loop →
  /// down) are pattern-derived, not per-sign.
  ///
  ...
  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        case STOP -> STABLE;
        case FAIL, KILL -> DEGRADED;
        case CRASH -> DEFECTIVE;
        case SPAWN, START, RESTART,
             SUSPEND, RESUME -> null;
      }
    );

  /// Canonical sign-to-kind classification for processes — each [Sign] tagged [Kind#OPERATION] or
  /// [Kind#OUTCOME]: the spawn/start lifecycle, supervisory `RESTART`, the forced `KILL`, and
  /// suspend/resume are operations; clean stop, failure exit, and crash are outcomes. `STOP` is an
  /// `OUTCOME` here (clean exit), unlike the Services `STOP` bracket; `KILL` is an `OPERATION` though
  /// [#STATUS] reads it `DEGRADED` (see [Kind]). Exhaustive without a `default`.

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case STOP, FAIL, CRASH -> Kind.OUTCOME;
        case SPAWN, START, KILL,
             RESTART, SUSPEND, RESUME -> Kind.OPERATION;
      }
    );

  /// Sign-to-[Operations] classification — the **episode structure** of each sign (see `SEQUENCERS.md`): a
  /// spawn opens the process span ([Operations.Sign#BEGIN]); start, restart, and suspend/resume advance it
  /// ([Operations.Sign#ADVANCE]); a kill, stop, failure, or crash ends it ([Operations.Sign#END]). Total and exhaustive
  /// without a `default`.

  public static final SignMap < Sign, Operations.Sign > OPERATION =
    SIGNS.map (
      sign -> switch ( sign ) {
        case SPAWN -> Operations.Sign.BEGIN;
        case START, RESTART, SUSPEND, RESUME -> Operations.Sign.ADVANCE;
        case KILL, STOP, FAIL, CRASH -> Operations.Sign.END;
      }
    );

  /// Sign-to-[Outcomes] translation — the *outcome* half of the grammatical interlingua: a clean stop
  /// reads [Outcomes.Sign#SUCCESS]; a failure or crash reads [Outcomes.Sign#FAIL]. The operations
  /// abstain. Exhaustive without a `default`, consistent with [#KIND].

  public static final SignMap < Sign, Outcomes.Sign > OUTCOME =
    SIGNS.map (
      sign -> switch ( sign ) {
        case STOP -> Outcomes.Sign.SUCCESS;
        case FAIL, CRASH -> Outcomes.Sign.FAIL;
        case SPAWN, START, KILL,
             RESTART, SUSPEND, RESUME -> null;
      }
    );

  private Processes () { }

  /// Creates a Process instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the process
  /// @return a new Process instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Process instruments from a conduit.
  ///
  ...
  @SpecRef ( {"4.2", "registry:processes"} )
  public enum Sign
