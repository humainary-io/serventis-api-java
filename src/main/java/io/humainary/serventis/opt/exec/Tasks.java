...
/// - **CANCEL**: Task canceled before or during execution
...
/// | `CANCEL`   | Task canceled (explicit cancellation request)                  |
/// | `TIMEOUT`  | Task exceeded execution time budget                             |
///
...
/// - High REJECT rate → DEGRADED status (capacity exhausted)
/// - Many FAIL signals → DEGRADED status (reliability issues)
/// - TIMEOUT pattern → DIVERGING status (performance degradation)
/// - Rising queue depth → DIVERGING status (approaching limits)
///
/// ### Task → Situation Assessment
/// - Queue depth explosion + REJECT → WARNING situation (scale out needed)
/// - Cascading FAIL pattern → CRITICAL situation (downstream dependency failed)
/// - TIMEOUT during transaction → CRITICAL situation (partial updates risk)
/// - High CANCEL rate → WARNING situation (excessive interruption)
///
...
public final class Tasks
  ...
  /// Canonical sign-to-status translation for tasks — the default *immediate interpretant* of the
  /// upward ascent (compose to override; see [SignMap]). A completed task reads healthy; failures,
  /// rejections, and timeouts read degraded; the submission/scheduling/execution lifecycle and
  /// cancellation abstain. Trend readings the prose lists (`TIMEOUT` → diverging, rising queue depth)
  /// are pattern-derived, not per-sign.
  ///
  ...
  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        ...
        case FAIL, REJECT, TIMEOUT -> DEGRADED;
        case SUBMIT, SCHEDULE, START, PROGRESS,
             SUSPEND, RESUME, CANCEL -> null;
      }
    );

  /// Canonical sign-to-kind classification for tasks — each [Sign] tagged [Kind#OPERATION] or
  /// [Kind#OUTCOME]: submission, scheduling, the start/progress lifecycle, suspend/resume, and
  /// cancellation are operations; rejection, completion, failure, and timeout are outcomes.
  /// Exhaustive without a `default`. See [Kind].

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case REJECT, COMPLETE, FAIL, TIMEOUT -> Kind.OUTCOME;
        case SUBMIT, SCHEDULE, START, PROGRESS,
             SUSPEND, RESUME, CANCEL -> Kind.OPERATION;
      }
    );

  /// Sign-to-[Operations] classification — the **episode structure** of each sign (see `SEQUENCERS.md`): a
  /// submit opens the task span ([Operations.Sign#BEGIN]); scheduling, starting, progress, and suspend/resume
  /// advance it ([Operations.Sign#ADVANCE]); a cancel, rejection, completion, failure, or timeout ends it
  /// ([Operations.Sign#END]). Total and exhaustive without a `default`.

  public static final SignMap < Sign, Operations.Sign > OPERATION =
    SIGNS.map (
      sign -> switch ( sign ) {
        case SUBMIT -> Operations.Sign.BEGIN;
        case SCHEDULE, START, PROGRESS, SUSPEND, RESUME -> Operations.Sign.ADVANCE;
        case CANCEL, REJECT, COMPLETE, FAIL, TIMEOUT -> Operations.Sign.END;
      }
    );

  /// Sign-to-[Outcomes] translation — the *outcome* half of the grammatical interlingua: completion
  /// reads [Outcomes.Sign#SUCCESS]; rejection, failure, and timeout read [Outcomes.Sign#FAIL]. The
  /// operations abstain. Exhaustive without a `default`, consistent with [#KIND].

  public static final SignMap < Sign, Outcomes.Sign > OUTCOME =
    SIGNS.map (
      sign -> switch ( sign ) {
        case COMPLETE -> Outcomes.Sign.SUCCESS;
        case REJECT, FAIL, TIMEOUT -> Outcomes.Sign.FAIL;
        case SUBMIT, SCHEDULE, START, PROGRESS,
             SUSPEND, RESUME, CANCEL -> null;
      }
    );
  ...
  /// Creates a Task instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the task
  /// @return a new Task instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Task instruments from a conduit.
  ///
  ...
  public enum Sign
    ...
    ///
    /// **Next states**: START (execution begins), CANCEL (canceled before start)
    ...
    ///
    /// **Next states**: RESUME (ready again), CANCEL (canceled while suspended), TIMEOUT
