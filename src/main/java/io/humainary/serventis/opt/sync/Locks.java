...
/// - High CONTEST/DENY rate → DEGRADED status (lock heavily contended)
/// - TIMEOUT pattern → DIVERGING status (synchronization delays)
...
/// - Rising hold times + contention → WARNING situation (redesign required)
/// - ABANDON during transaction → CRITICAL situation (potential data corruption)
///
...
public final class Locks
  ...
  /// Canonical sign-to-status translation for locks — the default *immediate interpretant* of the
  /// upward ascent (compose to override; see [SignMap]). Grants read healthy; denials, timeouts, and
  /// contention read degraded; an abandoned lock (crash without release) reads defective; the
  /// request, release, and upgrade/downgrade operations abstain. The trend readings this file's prose
  /// also lists (`TIMEOUT` → diverging, oscillation → erratic) are pattern-derived, not per-sign —
  /// future pattern-operator territory, not this level map.
  ///
  ...
  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        case GRANT -> STABLE;
        case DENY, TIMEOUT, CONTEST -> DEGRADED;
        case ABANDON -> DEFECTIVE;
        case ATTEMPT, ACQUIRE, RELEASE,
             UPGRADE, DOWNGRADE -> null;
      }
    );

  /// Canonical sign-to-kind classification for locks — each [Sign] tagged [Kind#OPERATION] or
  /// [Kind#OUTCOME]: the attempt/acquire/release request actions and the upgrade/downgrade
  /// modifications are operations; the grant/deny/timeout verdicts, contention detection, and
  /// abandonment are outcomes. Exhaustive without a `default`. See [Kind].

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case GRANT, DENY, TIMEOUT,
             CONTEST, ABANDON -> Kind.OUTCOME;
        case ATTEMPT, ACQUIRE, RELEASE,
             UPGRADE, DOWNGRADE -> Kind.OPERATION;
      }
    );

  /// Sign-to-[Operations] classification for locks — the **episode structure** of each sign (see
  /// `SEQUENCERS.md`). A try-attempt or a blocking acquire opens the span ([Operations.Sign#BEGIN]); a granted
  /// hold, a contention retry, and an upgrade/downgrade progress within it ([Operations.Sign#ADVANCE]); a release,
  /// denial, timeout, or abandonment ends it ([Operations.Sign#END]). Total and exhaustive without a `default` —
  /// every sign has an episode role, regardless of its [#KIND].

  public static final SignMap < Sign, Operations.Sign > OPERATION =
    SIGNS.map (
      sign -> switch ( sign ) {
        case ATTEMPT, ACQUIRE -> Operations.Sign.BEGIN;
        case GRANT, CONTEST, UPGRADE, DOWNGRADE -> Operations.Sign.ADVANCE;
        case DENY, TIMEOUT, RELEASE, ABANDON -> Operations.Sign.END;
      }
    );

  /// Sign-to-[Outcomes] translation for locks — the *outcome* half of the grammatical interlingua. It
  /// projects each verdict onto the universal result polarity: a grant reads [Outcomes.Sign#SUCCESS];
  /// denial, timeout, and abandonment read [Outcomes.Sign#FAIL]; contention reads [Outcomes.Sign#UNKNOWN]
  /// (a retry, not yet settled). The operations abstain — partial by design. Exhaustive without a
  /// `default`, and consistent with [#KIND]: every translated sign is [Kind#OUTCOME].

  public static final SignMap < Sign, Outcomes.Sign > OUTCOME =
    SIGNS.map (
      sign -> switch ( sign ) {
        case GRANT -> SUCCESS;
        case DENY, TIMEOUT, ABANDON -> FAIL;
        case CONTEST -> Outcomes.Sign.UNKNOWN;
        case ATTEMPT, ACQUIRE, RELEASE,
             UPGRADE, DOWNGRADE -> null;
      }
    );
  ...
  /// Creates a Lock instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the lock
  /// @return a new Lock instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Lock instruments from a conduit.
  ///
