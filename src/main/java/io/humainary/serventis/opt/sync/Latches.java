...
/// - Long AWAIT durations → DIVERGING status (slow coordination)
/// - Frequent RESET with short phases → ERRATIC status (thrashing)
///
/// ### Status → Situation Assessment
/// - Timeout pattern (many blocked AWAITs) → CRITICAL situation (coordination deadlock)
/// - ABANDON during critical phase → CRITICAL situation (incomplete coordination)
/// - Rising barrier durations → WARNING situation (coordination bottleneck)
///
...
public final class Latches
  ...
  /// Canonical sign-to-status translation for latches — the default *immediate interpretant* of the
  /// upward ascent (compose to override; see [SignMap]). A satisfied barrier (`RELEASE`) reads
  /// healthy; timeouts read degraded; an abandoned participant reads defective; waiting, arrival, and
  /// reset abstain. Note `RELEASE` reads *healthy* here — it is the success outcome of coordination,
  /// not the return-a-unit sense it carries in Resources/Locks — which is exactly why each domain
  /// authors its own map.
  ///
  ...
  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        case RELEASE -> STABLE;
        case TIMEOUT -> DEGRADED;
        case ABANDON -> DEFECTIVE;
        case AWAIT, ARRIVE, RESET -> null;
      }
    );

  /// Canonical sign-to-kind classification for latches — each [Sign] tagged [Kind#OPERATION] or
  /// [Kind#OUTCOME]: the await/arrive/reset participation actions are operations; the barrier
  /// `RELEASE`, `TIMEOUT`, and `ABANDON` are outcomes. Note `RELEASE` is an `OUTCOME` here (the
  /// barrier was satisfied), unlike the resource/lock `RELEASE` operation. Exhaustive without a
  /// `default`. See [Kind].

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case RELEASE, TIMEOUT, ABANDON -> Kind.OUTCOME;
        case AWAIT, ARRIVE, RESET -> Kind.OPERATION;
      }
    );

  /// Sign-to-[Operations] classification — the **episode structure** of each sign (see `SEQUENCERS.md`): the
  /// await opens the wait span ([Operations.Sign#BEGIN]); an arrival and a reset progress within it ([Operations.Sign#ADVANCE]);
  /// the barrier releasing, a timeout, or abandonment ends it ([Operations.Sign#END]). Total and exhaustive
  /// without a `default`.

  public static final SignMap < Sign, Operations.Sign > OPERATION =
    SIGNS.map (
      sign -> switch ( sign ) {
        case AWAIT -> Operations.Sign.BEGIN;
        case ARRIVE, RESET -> Operations.Sign.ADVANCE;
        case RELEASE, TIMEOUT, ABANDON -> Operations.Sign.END;
      }
    );

  /// Sign-to-[Outcomes] translation — the *outcome* half of the grammatical interlingua: the barrier
  /// releasing (the await satisfied) reads [Outcomes.Sign#SUCCESS]; a timeout or abandonment reads
  ...
  /// Creates a Latch instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the latch
  /// @return a new Latch instrument for the specified pipe
  /// @throws NullPointerException if the pipe parameter is `null`

  @SpecRef ( "6.4" )
  @New
  @NotNull
  public static Latch of (
  ...
  /// Returns a pool that creates cached Latch instruments from a conduit.
  ///
  ...
  public enum Sign
    ...
    /// AWAIT now proceed. Reuses the Resources RELEASE vocabulary, but classifies it as the
    /// success outcome (barrier satisfied) rather than a resource-return operation. Tracking
    /// time from first AWAIT to RELEASE reveals barrier duration.
    ///
