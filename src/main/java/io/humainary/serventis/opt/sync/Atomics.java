...
package io.humainary.serventis.opt.sync;
...
///   - High FAIL rate → DEGRADED status
///   - Frequent EXHAUST → DEFECTIVE status
...
/// - High FAIL/ATTEMPT ratio → DEGRADED status (severe contention)
/// - Frequent PARK signals → DEGRADED status (spinning insufficient)
/// - EXHAUST signals → DEFECTIVE status (retry budget exceeded)
/// - Low FAIL rate → STABLE status (healthy contention levels)
///
/// ### Status → Situation Assessment
/// - Sustained DEGRADED from atomics → WARNING situation (redesign needed)
/// - Rising EXHAUST rate → CRITICAL situation (operations failing)
/// - Sudden FAIL spike → WARNING situation (contention burst)
///
...
public final class Atomics
  ...
  /// Canonical sign-to-status translation for atomics — the default *immediate interpretant* of the
  /// upward ascent (compose to override; see [SignMap]). CAS success reads healthy; failure and
  /// escalation-to-blocking (`PARK`) read degraded; exhausting the retry budget reads defective; the
  /// attempt and the spin/yield/backoff retry behaviors abstain. A single `FAIL` is normal in
  /// lock-free code — the success/failure *ratio* the prose calls for emerges from the Scorecard's
  /// decayed plurality, not from any one sign.
  ///
  ...
  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        case SUCCESS -> STABLE;
        case FAIL, PARK -> DEGRADED;
        case EXHAUST -> DEFECTIVE;
        case ATTEMPT, SPIN, YIELD, BACKOFF -> null;
      }
    );

  /// Canonical sign-to-kind classification for atomics — each [Sign] tagged [Kind#OPERATION] or
  /// [Kind#OUTCOME]: the CAS `ATTEMPT` and the spin/yield/backoff/park contention responses are
  /// operations; success, the CAS-contention `FAIL`, and the exhausted-budget `EXHAUST` give-up are
  /// outcomes. `PARK` is an `OPERATION` though [#STATUS] reads it `DEGRADED` (see [Kind]). Exhaustive
  /// without a `default`.

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case SUCCESS, FAIL, EXHAUST -> Kind.OUTCOME;
        case ATTEMPT, SPIN, YIELD,
             BACKOFF, PARK -> Kind.OPERATION;
      }
    );

  /// Sign-to-[Operations] classification — the **episode structure** of each sign (see `SEQUENCERS.md`): the
  /// attempt opens the CAS span ([Operations.Sign#BEGIN]); a CAS `FAIL` and the spin/yield/backoff/park contention
  /// responses progress within it ([Operations.Sign#ADVANCE]); only a success or retry-budget exhaustion ends it
  /// ([Operations.Sign#END]). A CAS `FAIL` is a **mid-episode retry trigger, not a terminal** — the lifecycle is
  /// `ATTEMPT → FAIL → SPIN → FAIL → … → SUCCESS`/`EXHAUST`, so `FAIL` advances (it is `KIND=OUTCOME` yet a mid-span
  /// `ADVANCE`; `OPERATION` is cross-kind). Total and exhaustive without a `default`.

  public static final SignMap < Sign, Operations.Sign > OPERATION =
    SIGNS.map (
      sign -> switch ( sign ) {
        case ATTEMPT -> Operations.Sign.BEGIN;
        case FAIL, SPIN, YIELD, BACKOFF, PARK -> Operations.Sign.ADVANCE;
        case SUCCESS, EXHAUST -> Operations.Sign.END;
      }
    );

  /// Sign-to-[Outcomes] translation — the *outcome* half of the grammatical interlingua: a CAS success
  /// reads [Outcomes.Sign#SUCCESS]; retry-budget exhaustion reads [Outcomes.Sign#FAIL]; a CAS failure
  /// reads [Outcomes.Sign#UNKNOWN] — it is contention that will be retried, not a settled defeat, so it
  /// abstains from the success/failure tally rather than forcing a polarity (the lock-free twin of a
  /// lock `CONTEST`). The operations map to `null`. Exhaustive without a `default`, consistent with
  /// [#KIND].

  public static final SignMap < Sign, Outcomes.Sign > OUTCOME =
    SIGNS.map (
      sign -> switch ( sign ) {
        case SUCCESS -> Outcomes.Sign.SUCCESS;
        case EXHAUST -> Outcomes.Sign.FAIL;
        case FAIL -> Outcomes.Sign.UNKNOWN;
        case ATTEMPT, SPIN, YIELD,
             BACKOFF, PARK -> null;
      }
    );
  ...
  /// Creates an Atomic instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the atomic
  /// @return a new Atomic instrument for the specified pipe
  /// @throws NullPointerException if the pipe parameter is `null`

  @SpecRef ( "6.4" )
  @New
  @NotNull
  public static Atomic of (
  ...
  /// Returns a pool that creates cached Atomic instruments from a conduit.
  ///
