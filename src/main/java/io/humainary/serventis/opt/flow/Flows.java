...
///
/// ```java
/// var flow = circuit.conduit(Signal.class)
///   .pool(Flows::of).get(cortex.name("queue.orders"));
///
/// // Message received
...
/// ```java
/// var flow = circuit.conduit(Signal.class)
///   .pool(Flows::of).get(cortex.name("pipeline.etl"));
///
...
/// ```java
/// var flow = circuit.conduit(Signal.class)
///   .pool(Flows::of).get(cortex.name("http.requests"));
///
/// // Request received
...
/// |---------------------------|-----------|----------------------------------|
/// | High SUCCESS across all   | STABLE    | Flow moving smoothly             |
/// | High FAIL at INGRESS      | DEGRADED  | System refusing incoming work    |
/// | High FAIL at TRANSIT      | DEGRADED  | Processing failures occurring    |
/// | High FAIL at EGRESS       | DEGRADED  | Output failures occurring        |
/// | FAIL rate increasing      | DIVERGING | System health declining          |
///
/// ### Status -> Situation Assessment
///
/// | Status Pattern            | Situation | Example                          |
/// |---------------------------|-----------|----------------------------------|
/// | DEGRADED (ingress)        | WARNING   | Insufficient ingress capacity    |
/// | DEGRADED (processing)     | CRITICAL  | Processing pipeline broken       |
/// | DEGRADED (output)         | CRITICAL  | Downstream unavailable           |
///
...
public final class Flows
  ...
  /// flow signal instances and caller-derived signal maps.
  ...
  /// Canonical sign-to-status translation for flows — the default *immediate interpretant* of the
  /// upward ascent (sign-keyed; the INGRESS/TRANSIT/EGRESS stage does not change the reading; compose
  /// to override, see [SignMap]). A successful stage transition reads healthy, a failed one degraded.
  /// The success/fail *ratio* emerges from the Scorecard's plurality; per-stage assessment (a TRANSIT
  /// failure is graver than an INGRESS one) is the demux's job, not this map's. The instrument emits
  /// `Signal`, so as a Scorecards ballot project the sign first —
  ...
  /// Canonical sign-to-kind classification for flows — both signs are [Kind#OUTCOME]: `SUCCESS`/`FAIL`
  /// are stage-transition verdicts (Flows reports stage outcomes, not the operations that produced
  /// them — see Pipelines for those). The INGRESS/TRANSIT/EGRESS stage does not change the kind.
  /// Exhaustive without a `default`. See [Kind].

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case SUCCESS, FAIL -> OUTCOME;
      }
    );

  private Flows () { }

  /// Creates a Flow instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the flow
  /// @return a new Flow instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Flow instruments from a conduit.
  ///
  ...
  ///
  /// A flow is an instrument that emits signals composed of Sign x Dimension,
