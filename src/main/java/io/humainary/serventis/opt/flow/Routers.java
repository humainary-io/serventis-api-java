...
/// they traverse routing infrastructure. Magnitude and intensity are derived by observers
...
/// - **DROP reasons**: Observers can correlate DROP patterns with congestion, policy
...
/// - **Statuses API**: Per-sign — FORWARD → STABLE, DROP → DEGRADED, CORRUPT → DEFECTIVE; high REORDER → ERRATIC (trend)
///
/// ## Usage Example
///
/// ```java
/// final var cortex = Substrates.cortex();
/// var router = circuit.conduit(Sign.class).pool(Routers::of).get(cortex.name("edge01.eth0"));
///
/// // Packet arrives
/// router.receive();
///
...
public final class Routers
  ...
  /// Canonical sign-to-status translation for routers — the default *immediate interpretant* of the
  /// upward ascent (compose to override; see [SignMap]). `FORWARD` (packet routed onward) reads
  /// healthy, `DROP` (packet lost) degraded, `CORRUPT` (integrity failure) defective; the
  /// send/receive/route lifecycle, fragment/reassemble handling, and `REORDER` abstain — `REORDER`'s
  /// "high reorder → erratic" is a trend, for the sequencer. The forward/drop *ratio* — delivery
  /// success rate — emerges from the Scorecard's plurality.
  ///
  ...
  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        case FORWARD -> STABLE;
        case DROP -> DEGRADED;
        case CORRUPT -> DEFECTIVE;
        case SEND, RECEIVE, ROUTE,
             FRAGMENT, REASSEMBLE, REORDER -> null;
      }
    );

  /// Canonical sign-to-kind classification for routers — each [Sign] tagged [Kind#OPERATION] or
  /// [Kind#OUTCOME]: the packet-handling actions (send/receive/forward/route/drop/fragment/reassemble)
  /// are operations; the detected `CORRUPT`/`REORDER` anomalies are outcomes. `FORWARD` and `DROP` are
  /// operations though [#STATUS] reads them (`STABLE`/`DEGRADED`) — act/result is independent of the
  /// reading (see [Kind]). Exhaustive without a `default`.

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case CORRUPT, REORDER -> OUTCOME;
        case SEND, RECEIVE, FORWARD, ROUTE,
             DROP, FRAGMENT, REASSEMBLE -> OPERATION;
      }
    );

  private Routers () { }

  /// Creates a Router instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the router
  /// @return a new Router instrument for the specified pipe
  ...
  /// Returns a pool that creates cached Router instruments from a conduit.
  ///
  ...
  public enum Sign
    ...
    /// issues, policy enforcement, or routing problems. Observers can correlate drops with
