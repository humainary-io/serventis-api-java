...
/// - **Statuses API**: Breaker state translates per-sign — OPEN → DEFECTIVE (backend predominantly failing), TRIP → DEGRADED (break onset), CLOSE → STABLE
...
public final class Breakers
  ...
  /// Canonical sign-to-status translation for breakers — the default *immediate interpretant* of the
  /// upward ascent (compose to override; see [SignMap]). A closed circuit reads healthy; the trip that
  /// breaks the circuit reads degraded (the failure onset); an open circuit reads defective (the
  /// backend is predominantly failing — the reason the breaker opened); the recovery-testing states
  /// (`HALF_OPEN`, `PROBE`) and the manual `RESET` abstain.
  ///
  /// Breaker signs are **state transitions**, not a frequency stream: read this as a direct
  /// latest-state translation — `STATUS.apply(sign)` — and do **not** route it through
  /// `Scorecards.flow`. A decayed plurality would still report `STABLE` after many `CLOSE` votes when
  /// a single `OPEN` means the circuit is now open; a state/latest-value operator is the correct
  /// Scorecard-style translator for this domain.
  ///
  ...
  public static final SignMap < Sign, Statuses.Sign > STATUS =
    SIGNS.map (
      sign -> switch ( sign ) {
        ...
        case TRIP -> DEGRADED;
        case OPEN -> DEFECTIVE;
        case HALF_OPEN, PROBE, RESET -> null;
      }
    );

  /// Canonical sign-to-kind classification for breakers — each [Sign] tagged [Kind#OPERATION] or
  /// [Kind#OUTCOME]: the breaker states (`CLOSE`/`OPEN`/`HALF_OPEN`) and the threshold-breach `TRIP`
  /// are outcomes — conditions read directly into status as a latest-state reading; the test `PROBE`
  /// and manual `RESET` are operations. Exhaustive without a `default`. See [Kind].

  public static final SignMap < Sign, Kind > KIND =
    SIGNS.map (
      sign -> switch ( sign ) {
        case CLOSE, OPEN, HALF_OPEN, TRIP -> OUTCOME;
        case PROBE, RESET -> OPERATION;
      }
    );

  private Breakers () { }

  /// Creates a Breaker instrument wrapping the specified pipe.
  ///
  /// @param pipe the pipe from which to create the breaker
  /// @return a new Breaker instrument for the specified pipe
  /// @throws NullPointerException if the pipe parameter is `null`

  @SpecRef ( "6.4" )
  @New
  @NotNull
  public static Breaker of (
  ...
  /// Returns a pool that creates cached Breaker instruments from a conduit.
  ///
