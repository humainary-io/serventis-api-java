...
Serventis is an extension module built on Humainary Substrates that provides comprehensive
observability primitives for distributed services. It enables structured monitoring, assessment, and
reporting of service health, performance, and interaction patterns through semantic signal emission.

## Specification

This repository is the **Java projection** of a language-neutral specification. The specification is
normative for portable behavior; this API is one way of expressing it.

| Document                                                                                   | Role                                                                                            |
|--------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------|
| [SPEC.md](https://github.com/humainary-io/serventis-api-spec/blob/3.6.0/SPEC.md)           | Normative: the observation model, instruments, universal vocabularies, translation, conformance |
| [REGISTRY.md](https://github.com/humainary-io/serventis-api-spec/blob/3.6.0/REGISTRY.md)   | Normative membership: every registered vocabulary's signs and dimensions, fixed once published  |
| [RATIONALE.md](https://github.com/humainary-io/serventis-api-spec/blob/3.6.0/RATIONALE.md) | Non-normative: why the model is shaped as it is                                                 |

Serventis layers
over [Substrates](https://github.com/humainary-io/substrates-api-spec/blob/3.6.0/SPEC.md),
which governs circulation — emission, ordering, identity, lifecycle. This version requires
Substrates
3.6.0.

### What is portable and what is not

Not everything here is specified. Declarations carry
[traceability annotations](https://github.com/humainary-io/specs-api-java): `@SpecDoc` names the
governing document at a pinned tag, and `@SpecRef` names the clauses a declaration realizes.

**A type carrying `@SpecDoc` but no `@SpecRef` claims no portable coverage.** That is how this
projection marks what the specification leaves to an implementation:

- `SignMap`, `SignalMap`, and the precomputed `SignalSet` — execution affordances (SPEC Appendix
  A.2)
- The per-domain `STATUS`, `KIND`, `OPERATION`, and `OUTCOME` maps, plus `Scorecards`, `Sequencers`,
  and `Cycles.flow` — interpretive policy (SPEC Appendix B)

Those are useful and supported; they are not what another language's projection must reproduce to
conform. Portable declarations carry `@SpecRef`, and the
[TCK](https://github.com/humainary-io/serventis-api-java-tck) applies the same references to the
tests
that verify them.
...
- **Probes**: Low-level communication outcome observation (connect, transfer, process)
...
- **Statuses**: Operational condition assessment (stable, degraded, defective, down)
...
- **Sensors**: Positional measurement relative to configured reference values
- **Evals**: Criterion-qualified evaluation results (pass, fail, unknown, skip, error)
...
- **Outcomes**: Success/failure/unknown verdict vocabulary for universal aggregation
- **Operations**: Begin/advance/end episode bracket vocabulary for universal span tracking
...
│   ├── SymbolSet.java          - Reified symbol axis (Sign or Dimension constants)
│   ├── SignSet.java            - Reified sign axis and SignMap factory
│   ├── SignalSet.java          - Signal caching utility for Sign × Dimension
│   ├── SignMap.java            - Ordinal-indexed sign projection cache
│   ├── SignalMap.java          - Ordinal-indexed signal projection cache
│   ├── Scorecards.java         - Running/windowed status translation
...
│   ├── Outcomes.java           - SUCCESS/FAIL/UNKNOWN verdict vocabulary
│   ├── Operations.java         - BEGIN/ADVANCE/END episode bracket vocabulary
...
│       └── Sequencers.java     - Ordered sign-stream trajectory recognition
└── opt/                        - Domain vocabularies (optional)
    ...
    │   └── Changes.java        - Deployment and configuration change episodes
    ...
    │   └── Messages.java       - Message delivery and settlement
    ├── flow/                   - Flow control
    ...
    │   └── Guards.java         - Authentication and authorization decisions
    ...
    │   └── Members.java        - Group membership and leadership
    └── tool/                   - Measurement tools
        ...
        └── Evals.java          - Criterion-qualified evaluation results
```
...
- **Sign**: Principal semantic classification (what is being observed)
- **Dimension**: Secondary qualifier (perspective, stage, confidence, criterion, or directionality)
...
- **Evals**: Sign (PASS, FAIL, UNKNOWN, SKIP, ERROR) × Dimension (evaluation criterion)
- **Outcomes**: Sign (SUCCESS, FAIL, UNKNOWN) × No Dimension
- **Operations**: Sign (BEGIN, ADVANCE, END) × No Dimension
...
All Serventis instruments are created from substrate channels and emit through one of two base
interfaces defined on `Serventis`:
...
interface Signaler < S extends Enum< S > & Sign, D extends Enum< D > & Dimension > {
    void signal(S sign, D dimension);
}
```

**Key Concepts**:

- **Signer**: Base interface for one-dimensional emission. Used by APIs where a single sign carries
  the full semantics (Counters, Gauges, Resources, Tasks, Locks, Queues, ...).
- **Signaler**: Base interface for two-dimensional emission. Used by APIs that pair a sign with a
  qualifying dimension (Services, Transactions, Probes, Statuses, Agents, ...).
- **Substrate integration**: Instruments are produced from `Conduit` channels via domain-specific
  composers; emissions flow through the underlying `Pipe<E>`, which is itself a
  `Substrate<Pipe<E>>` in the substrates API.
- **Intentionality**: Conduit-derived instruments are pooled per name.
- **Semantic clarity**: Instruments provide domain semantics over raw pipe emissions.
- **Framework extensibility**: New instrument types adopt `Signer` or `Signaler` and reuse the
...
3. **Dual Orientation**: Perspective-based dimensions (OUTBOUND/INBOUND, CALLER/CALLEE,
   PROMISER/PROMISEE)
4. **Statistical Confidence**: Conditions include certainty measures (tentative, measured,
   confirmed)
5. **Substrate Integration**: Built on circuits and conduits for deterministic ordering
...
2. **Assertions & Explanations** (3 signs): AFFIRM, EXPLAIN, REPORT
...
- **Question-Answer**: ASK → EXPLAIN → AFFIRM → ACKNOWLEDGE
- **Request-Delivery**: REQUEST → ACKNOWLEDGE → PROMISE → DELIVER → ACKNOWLEDGE
- **Correction-Clarification**: AFFIRM → DENY → CLARIFY → ACKNOWLEDGE
...
- **Statuses API**: Actors AFFIRM status conditions about system state
...
- **Discovery**: INQUIRE → OFFER → OBSERVE
- **Commitment**: OFFER → PROMISE → ACCEPT
- **Dependency**: DEPEND → ACCEPT → PROMISE
- **Fulfillment**: PROMISE → FULFILL
- **Breach**: PROMISE → BREACH
- **Retraction**: PROMISE → RETRACT → ACCEPT
...
- **Saga Pattern**: START → COMPENSATE (after a step fails) → ROLLBACK
...
- **Statuses API**: Transaction patterns inform conditions (ABORT → DEFECTIVE,
  ROLLBACK/EXPIRE/CONFLICT → DEGRADED)
...
- **Statuses API**: Breaker state translates per-sign — OPEN → DEFECTIVE (backend predominantly
  failing), TRIP → DEGRADED (break onset), CLOSE → STABLE
...
entry (INGRESS), processing (TRANSIT), and exit (EGRESS). Each stage can succeed or fail, creating a
matrix of observable outcomes for throughput analysis, bottleneck detection, and failure
localization.
...
1. **Message Queue Processing**:
   ...
   ```java
   var conduit = circuit.conduit(Flows.Signal.class);
   var flows   = Flows.pool(conduit);
   var flow    = flows.get(cortex.name("queue.orders"));
...
2. **Data Pipeline**:
   ...
   var conduit = circuit.conduit(Flows.Signal.class);
   var flows   = Flows.pool(conduit);
   var flow    = flows.get(cortex.name("pipeline.etl"));
...
3. **Request Handler**:
   ...
   var conduit = circuit.conduit(Flows.Signal.class);
   var flows   = Flows.pool(conduit);
   var flow    = flows.get(cortex.name("http.requests"));
...
|-------------------------|----------|-----------|
| High SUCCESS across all | STABLE   | NORMAL    |
| High FAIL at INGRESS    | DEGRADED | WARNING   |
| High FAIL at TRANSIT    | DEGRADED | CRITICAL  |
| High FAIL at EGRESS     | DEGRADED | CRITICAL  |
...
**Performance**: Zero-allocation sign emission
...
- **Direction**: OUTBOUND (sending) or INBOUND (receiving)
- **Operation**: CONNECT, DISCONNECT, TRANSFER, PROCESS
...
**Performance**: High frequency for utilization tracking (BORROW/RECLAIM), low frequency for
capacity changes (EXPAND/CONTRACT)
...
**Performance**: Medium frequency, coordination timescales
...
**Performance**: Moderate frequency, observation timescales

---

### Outcomes API (sdk)

**Purpose**: Minimal verdict vocabulary (success / failure / unknown) for universal aggregation.

**Design Philosophy**: Outcomes provides the simplest possible verdict vocabulary. As signals ascend
the semiotic hierarchy, they trade specificity for universality. The cause is lost; the verdict
remains. If you need to know *why* something failed, subscribe to the domain API directly. Outcomes
answers one question: **did it work?**
...
| UNKNOWN | Verdict-bearing but indeterminate (polarity unsettled) |

**Total Signals**: 3 (no dimensions)

**Usage Example**:

```java
// Create an outcome instrument
var conduit = circuit.conduit(Outcomes.Sign.class);
var outcomes = Outcomes.pool(conduit);
var outcome = outcomes.get(cortex.name("payment.outcomes"));
...
- Success/failure health signals for Status translation
...
- **Outcomes**: Aggregates to a verdict (success / failure / unknown) for universal consumption
...
**Purpose**: Minimal episode-structure vocabulary (begin / advance / end) for universal span
tracking.

**Design Philosophy**: Every episode opens, may progress, and closes. Operations captures this
universal three-phase pattern. Combined with a quality reading (a domain's STATUS, or Outcomes for a
success/fail tally), you get the complete picture:

- BEGIN → END (episode opened and closed cleanly)
- BEGIN → ADVANCE → END (episode progressed through mid-span steps before closing)
- BEGIN → END with a degraded/defective close (episode closed unhealthily)

**Signs** (3 brackets):

| Sign    | Meaning                                            |
|---------|----------------------------------------------------|
| BEGIN   | Episode opening                                    |
| ADVANCE | Mid-episode step — neither opens nor closes a span |
| END     | Episode closing                                    |
...
var conduit = circuit.conduit(Operations.Sign.class);
var operations = Operations.pool(conduit);
var operation = operations.get(cortex.name("db.query"));
...
- **Operations**: Aggregates to a three-phase episode bracket for universal span tracking
...
detected in time-series observations, based on techniques like Nelson's 8 Rules for SPC. Combined
with Outcomes and Operations, you get the complete observability picture:

- **Operations**: BEGIN/ADVANCE/END (when did it run?)
- **Outcomes**: SUCCESS/FAIL/UNKNOWN (did it work?)
...
var conduit = circuit.conduit(Trends.Sign.class);
var trends = Trends.pool(conduit);
var trend = trends.get(cortex.name("api.latency"));
...
Conduit<Cycles.Signal<Resources.Sign>> conduit =
  (Conduit<Cycles.Signal<Resources.Sign>>) (Conduit<?>) circuit.conduit(Cycles.Signal.class);
var cycles = Cycles.pool(Resources.SIGNS, conduit);
var cycle = cycles.get(cortex.name("db.pool.cycles"));
...
**Design Philosophy**: While Statuses expresses individual assessment with confidence (how sure am
I?), Surveys expresses collective assessment with agreement (how much do we agree?). These are
complementary dimensions of observability. Like Cycles, Surveys is **generic over any Sign type** -
the Sign comes from the source API being surveyed (typically Statuses.Sign), and Surveys defines
only the Dimension describing the agreement level.
...
@SuppressWarnings("unchecked")
Conduit<Surveys.Signal<Statuses.Sign>> conduit =
  (Conduit<Surveys.Signal<Statuses.Sign>>) (Conduit<?>) circuit.conduit(Surveys.Signal.class);
var surveys = Surveys.pool(Statuses.SIGNS, conduit);
var survey = surveys.get(cortex.name("cluster.health"));
...
- **CALLER**: Calling service perspective ("I am calling the remote service")
- **CALLEE**: Called service perspective ("I am serving the incoming request")
...
| ABANDON   | Error        | Lock holder crashed without release     | Lock-specific |
...
**Performance**: Synchronization timescales (nanoseconds to milliseconds), zero-allocation sign
emission
...
(GRANT → RELEASE). Atomics is about contention dynamics (FAIL → SPIN → BACKOFF → SUCCESS). These
APIs complement each other: CAS-based lock acquisition can emit both Atomics signs (for contention
dynamics) and Locks signs (for ownership semantics).
...
**Performance**: Nanosecond timescales, zero-allocation sign emission
...
- **Statuses API**: Contention patterns inform conditions:
    - High FAIL/ATTEMPT ratio → DEGRADED status
    - Frequent PARK → DEGRADED status
    - EXHAUST signals → DEFECTIVE status
...
where appropriate (AWAIT, RELEASE, TIMEOUT) with latch-specific extensions (ARRIVE, RESET, ABANDON)
...
**Performance**: Coordination timescales (microseconds to milliseconds), zero-allocation sign
emission
...
- **Canceled Task**: SUBMIT → SCHEDULE → CANCEL or SUBMIT → SCHEDULE → START → CANCEL
...
**Performance**: Computational timescales (microseconds to seconds), zero-allocation sign emission
...
- **Situations API**: Queue depth explosion + REJECT → WARNING situation (scale out needed)
...
**Design Philosophy**: Timers provides a qualitative vocabulary for reporting whether execution met
or missed time constraints. Inspired by Apdex, the API focuses on binary outcomes (MEET/MISS)
rather than quantitative durations. The implementer determines the constraint; the API provides the
vocabulary for reporting the outcome. Pattern recognition and threshold interpretation happen in
subscribers, not at the emission point.
...
zero-allocation SignalSet caching
...
- **Statuses API**: Process termination patterns inform conditions (CRASH → DEFECTIVE, FAIL/KILL →
  DEGRADED)
...
- **Statuses API**: Stack overflow patterns may indicate DEGRADED or DIVERGING conditions
...
|-----------------------|-----------|-----------|
| High PUSH without POP | DIVERGING | WARNING   |
| Many OVERFLOW         | DEGRADED  | WARNING   |
| Many UNDERFLOW        | DEGRADED  | WARNING   |
...
**Performance**: Moderate frequency, metrics-based measurements
...
- Forms measurement foundation: Metrics → Sensors (positions) → Statuses (assessments) → Situations
  (urgency)
...
**Purpose**: Criterion-qualified qualitative evaluation results for a named subject

**Results** (5 signs):

| Sign    | Meaning                                            |
|---------|----------------------------------------------------|
| PASS    | The subject satisfied the criterion                |
| FAIL    | The subject violated the criterion                 |
| UNKNOWN | Evaluation completed without a determinate verdict |
| SKIP    | The criterion was deliberately not evaluated       |
| ERROR   | Evaluation was attempted but could not complete    |

**Criteria** (8 dimensions): what the unit of work produced — CORRECTNESS, COMPLETENESS,
GROUNDEDNESS, HELPFULNESS, ADHERENCE, SAFETY — and how it produced it — TOOLING, ROUTING.

The criteria are deliberately non-overlapping, so a single defect fires exactly one of them and an
observer weighting the set never counts it twice. HELPFULNESS subsumes mere relevance; ADHERENCE
excludes safety policy, which SAFETY judges. A criterion is admitted only when an observer would
weight it differently from every criterion already present.

**Total Signals**: 40 signals (5 signs × 8 dimensions)

**Key Distinctions**:

- **UNKNOWN vs SKIP**: UNKNOWN records an indeterminate verdict after evaluation; SKIP records a
  deliberate decision not to evaluate
- **ERROR vs FAIL**: ERROR means the judge could not complete; FAIL means the judge completed and
  the subject violated the criterion
- **SKIP vs silence**: SKIP is an observed harness result; silence means no evaluation event was
  observed

**Semiotic Ascent**: PASS → Outcomes.SUCCESS, FAIL → Outcomes.FAIL, UNKNOWN/ERROR →
Outcomes.UNKNOWN, while SKIP abstains. For status ballots, PASS → STABLE, FAIL → DEGRADED, while
UNKNOWN, SKIP, and ERROR abstain.
...
subject. The subject is never the evaluator. ERROR is the one result that says nothing about the
unit of work — a judge that timed out reports a defect in the evaluator, not in what it was
evaluating — so ERROR abstains from the status reading and does not count as a failure of the
subject in verdict tallies. Report evaluator health against the evaluator's own subject.

**Weighting Criteria**: the criterion set is closed so an observer can weight criteria against each
other — a safety failure and a completeness failure are not the same event. The canonical STATUS map
is sign-keyed and reads every criterion alike; cross `SIGNS` with `DIMENSIONS` to key on the whole
signal instead:

```java
static final SignalMap< Sign, Dimension, Signal, Statuses.Sign > WEIGHTED =
  Evals.SIGNS.signals(Evals.DIMENSIONS, Signal::new)
    .map(
...
The projection runs once per signal when the map is built; each lookup is a single array load
indexed by the two ordinals. Weightings belong here, not in the canonical STATUS/KIND/OUTCOME maps,
which key on the sign alone by design.

---

### Messages API

**Purpose**: Delivery-contract signaling for brokers, event streams and consumers: whether a message
was acknowledged, refused, delivered again, set aside, or allowed to expire

**Signs** (7 signs):

| Sign      | Meaning                                                                                |
|-----------|----------------------------------------------------------------------------------------|
| PUBLISH   | A message was handed on for delivery                                                   |
| DELIVER   | A message was handed to a receiver                                                     |
| ACK       | The receiving end confirmed the message                                                |
| NACK      | The receiving end refused or failed the message                                        |
| REDELIVER | A message was delivered again because an earlier delivery was refused or never settled |
| EXHAUST   | A message's delivery attempts ran out, and it was set aside or dropped                 |
| EXPIRE    | A message's time to live ran out before delivery                                       |

**Dimensions** (3 perspectives): PRODUCER, BROKER, CONSUMER — the party reporting

**Total Signals**: 21 signals (7 signs × 3 dimensions)

```java
message.publish(PRODUCER);
message.ack(PRODUCER);        // publish confirmed
message.deliver(BROKER);
message.nack(CONSUMER);       // consumer refused it
message.redeliver(BROKER);
message.exhaust(BROKER);      // delivery attempts ran out
```

**Properties**: `KIND` and `STATUS` (ACK stable; NACK and EXPIRE degraded; EXHAUST defective)

**Relationship to Other APIs**:

- **Queues API**: a queue reports its boundary; Messages reports whether delivery was accepted
- **Services API**: request/response calls belong to Services

---

### Members API

**Purpose**: Group membership and leadership as the membership protocol decides them

**Signs** (7 signs):

| Sign    | Meaning                                      |
|---------|----------------------------------------------|
| JOIN    | A member joined the group                    |
| LEAVE   | A member left the group gracefully           |
| SUSPECT | The failure detector marked a member suspect |
| REFUTE  | A suspected member proved it was alive       |
| EVICT   | A member was declared failed and removed     |
| ELECT   | A member became leader                       |
| RESIGN  | A leader stepped down                        |

**Dimensions** (2 perspectives): SELF (reported by the member concerned), PEER (reported by another
member)

**Total Signals**: 14 signals (7 signs × 2 dimensions)

`SUSPECT` and `EVICT` must report the failure detector's own decision. A producer that computes
suspicion from heartbeat gaps itself is deriving it, which this primary vocabulary does not carry.
`EVICT × SELF` means this member was removed while still running, which is how split brain first
shows.

**Properties**: `KIND` and `STATUS` (REFUTE and ELECT stable; SUSPECT degraded; EVICT abstains
because its reading depends on the dimension)

**Relationship to Other APIs**:

- **Leases API**: a leader's lease is reported through Leases; who is in the group is reported here

---

### Changes API

**Purpose**: Deployment, configuration and flag changes as episodes, so a degraded subject can be
correlated with what changed

**Signs** (6 signs):

| Sign     | Meaning                                                          |
|----------|------------------------------------------------------------------|
| START    | A change to the subject began                                    |
| APPLY    | The change took effect                                           |
| FAIL     | The change could not be applied                                  |
| REJECT   | The subject refused the change, by validation or policy          |
| REVERT   | A change still rolling out was rolled back before it took effect |
| PROGRESS | The change advanced a stage, such as a canary step               |

`START` opens the episode, `PROGRESS` marks each stage of a staged rollout, and exactly one of
`APPLY`, `FAIL`, `REJECT` or `REVERT` closes it. Validation is part of the
change, so a refusal before rollout still follows `START`. Undoing a change that already took effect
is a new change. Each subject carries one open change at a time: changes that can overlap go under
distinct subjects, and a change abandoned for a newer one is closed first.
...
var trajectory = Sequencers.flow(Changes.OPERATION, Changes.STATUS);
```

**Properties**: `KIND`, `STATUS`, `OPERATION` and `OUTCOME` (APPLY stable; FAIL and REVERT
degraded; REJECT abstains, since refusing a bad change is the system working). FAIL, REJECT and
REVERT all read as failures in outcome tallies.

---

### Guards API

**Purpose**: Authentication and authorization decisions at a guard such as an authenticator or a
...
| Sign      | Meaning                                             |
|-----------|-----------------------------------------------------|
| ATTEMPT   | A credential or request was presented               |
| CHALLENGE | Further proof was required, such as a second factor |
| GRANT     | Access was allowed                                  |
| DENY      | Access was refused                                  |

**Dimensions** (2 checks): IDENTITY (authentication), PERMISSION (authorization)

**Total Signals**: 8 signals (4 signs × 2 dimensions)
...
guard.deny(PERMISSION);       // a known caller without the right
```

**Properties**: `KIND`, `STATUS` and `OUTCOME` (GRANT stable; DENY degraded). A Scorecard weighs
recent votes most, so it reads the guard degraded once refusals outweigh recent grants; with little
history the reading is tentative, and one refusal can tip it. No `OPERATION`, because a guard serves many
requests at once. A session or token a guard issues is a lease; report it through Leases. An
unreachable identity store is reported on its own subject through Services or Probes; the guard
reports what it decided, DENY when it fails closed.
...
Serventis is built on Substrates primitives with the Conduit/Lookup pattern:

- **Conduits**: `Conduit<E>` — pools named pipes by emission type (Sign or Signal)
- **Pools**: `Xxx.pool(conduit)` — creates cached instrument views per name
- **Pipes**: `Pipe<E> extends Substrate<Pipe<E>>` — primitive emission mechanism
- **Subscribers**: React to signals with aggregation logic

All signal emission leverages Substrates' **transit-priority cascading order** to ensure cascading
effects complete before later ingress work is observed.

**Instrument Creation Pattern**: Each Serventis API provides a static `pool()` method that wraps a
conduit in a cached derived view. The resulting `Pool<Instrument>` creates instruments on first
access by name and returns the cached instance on subsequent accesses.

## Performance Characteristics

Instruments differ in the rate their domain naturally produces, not in the cost of emitting:

- **High-frequency**: counters, gauges, caches, services — emitted per operation
- **Request-level**: probes — emitted per request
- **Condition-level**: statuses — emitted per assessment
- **Situational**: reporters — emitted per narrative update

Every domain travels the same path, so the emission cost is the same for all of them. Signs are
cached enum-backed values, so emission allocates nothing; a sign cascading within the circuit thread
takes the transit queue and is cheaper than one admitted from an external thread.

Rates are deliberately not published here. They depend on the host, the JDK, and the SPI provider,
and a figure written into a README outlives all three. Measure with
[Perfkit](https://github.com/humainary-io/perfkit-java), whose `BENCHMARKS.md` sets out what a
result must satisfy before it means anything.

## Building

The pinned Maven wrapper is included, so a system Maven installation is not required:

```bash
./mvnw clean verify
```

This repository publishes **interface surface only**. It carries no runtime: behavior comes from a
Substrates SPI provider you supply, and nothing here builds or installs one.

Two artifacts must be resolvable to compile it:

- `io.humainary.substrates:humainary-substrates-api`
- `io.humainary.specs:humainary-specs-api`, at `provided` scope for the traceability annotations

Both default to the version in `pom.xml` and can be overridden with `-Dsubstrates.api.version=` and
`-Dspecs.api.version=`.

## Conformance and measurement

Conformance and measurement each live in their own repository. Both take your provider's Maven
coordinates and build nothing they test or measure.

- **[serventis-api-java-tck](https://github.com/humainary-io/serventis-api-java-tck)** — executable
  conformance tests for this API on a Substrates provider. Its README covers provider coordinates
  and discovery.
- **[perfkit-java](https://github.com/humainary-io/perfkit-java)** — JMH measurement of a provider's
  steady-state cost, including the semantic ascent layer.
