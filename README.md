...
Serventis is an extension module built on Humainary Substrates that provides
...
- **Probes**: Sign (CONNECT, TRANSMIT, etc.) × Dimension (OUTBOUND, INBOUND)
...
All Serventis instruments are created from substrate channels and emit through one of two
base interfaces defined on `Serventis`:
...
- **Signer**: Base interface for one-dimensional emission. Used by APIs where a single sign
  carries the full semantics (Counters, Gauges, Resources, Tasks, Locks, Queues, ...).
- **Signaler**: Base interface for two-dimensional emission. Used by APIs that pair a sign with
  a qualifying dimension (Services, Transactions, Probes, Statuses, Agents, ...).
...
1. **Message Queue Processing**:
   ...
   var flows   = Flows.lookup(conduit);
...
var outcomes = Outcomes.lookup(conduit);
...
var operations = Operations.lookup(conduit);
...
var trends = Trends.lookup(conduit);
...
var cycles = Cycles.lookup(Resources.Sign.class, conduit);
...
var surveys = Surveys.lookup(Statuses.Sign.class, conduit);
var survey = surveys.get(cortex.name("cluster.health"));

// After polling cluster members:
survey.

signal(DEGRADED, MAJORITY);   // 4 of 5 nodes say DEGRADED
survey.

signal(STABLE, UNANIMOUS);    // All nodes agree STABLE
survey.

signal(DIVERGING, DIVIDED);   // Observers split on assessment
```
...
**Instrument Creation Pattern**: Each Serventis API provides a static `pool()` method that
wraps a conduit in a cached derived view. The resulting `Pool<Instrument>` creates instruments
on first access by name and returns the cached instance on subsequent accesses.
...
**Note**: Tests are skipped by default. Use `./tck.sh` or `./build.sh -t` from the project root
to enable testing. See `substrates/private/README.md` for more details on TCK configuration.
