...
/// - High MEET rate → STABLE status (system meeting time constraints)
/// - Rising MISS rate → DIVERGING status (time constraints being violated)
/// - Sustained high MISS rate → DEFECTIVE status (chronic time constraint failure)
///
/// ### Status → Situation Assessment
/// - DIVERGING from THRESHOLD misses → WARNING situation (need more resources)
/// - DEFECTIVE from DEADLINE misses → CRITICAL situation (immediate intervention)
...
public final class Timers
  ...
  /// Returns a lookup that creates cached Timer instruments from a conduit.
  ///
  /// @param conduit the conduit providing signal pipes
  /// @return a lookup that creates Timer instruments
