...
/// - **Statuses API**: Valve patterns (high DENY rate) inform Status conditions (DEGRADED)
...
///   .lookup(Valves::of).get(cortex.name("k8s.deployment.replicas"));
///
...
/// ```java
/// var valve = circuit.conduit(Sign.class)
///   .lookup(Valves::of).get(cortex.name("api.ratelimit"));
///
...
/// ```java
/// var valve = circuit.conduit(Sign.class)
///   .lookup(Valves::of).get(cortex.name("gateway.tokens"));
///
...
/// ```java
/// var valve = circuit.conduit(Sign.class)
///   .lookup(Valves::of).get(cortex.name("service.breaker"));
///
...
/// ```java
/// var valve = circuit.conduit(Sign.class)
///   .lookup(Valves::of).get(cortex.name("bulkhead.critical"));
///
...
/// ```java
/// var valve = circuit.conduit(Sign.class)
///   .lookup(Valves::of).get(cortex.name("gateway.dropder"));
///
...
/// | DROP activity | DEGRADED | Emergency load dropping active |
...
/// | DEGRADED (DROP active) | CRITICAL | System unable to serve traffic |
...
public final class Valves
  ...
  /// Returns a lookup that creates cached Valve instruments from a conduit.
  ///
  /// @param conduit the conduit providing sign pipes
  /// @return a lookup that creates Valve instruments
  ...
  public enum Sign
    ...
    ///
    /// **Pattern analysis**: DROP activity → DEGRADED status, DROP + CONTRACT → protecting system
