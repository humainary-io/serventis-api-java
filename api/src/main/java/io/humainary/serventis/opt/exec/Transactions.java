...
public final class Transactions
  ...
  /// start opens the transaction span ([Operations.Sign#BEGIN]); a prepare and a compensate advance it
  /// ([Operations.Sign#ADVANCE]); a commit, rollback, abort, expiry, or conflict ends it ([Operations.Sign#END]). Total and
  ...
  public static final SignMap < Sign, Operations.Sign > OPERATION =
    SIGNS.map (
      sign -> switch ( sign ) {
        ...
        case PREPARE, COMPENSATE -> Operations.Sign.ADVANCE;
        case COMMIT, ROLLBACK, ABORT, EXPIRE, CONFLICT -> Operations.Sign.END;
      }
    );
