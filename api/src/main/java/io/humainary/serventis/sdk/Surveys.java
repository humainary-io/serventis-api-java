...
/// var survey  = Surveys.lookup(Statuses.Sign.class, conduit).get(cortex.name("cluster.health"));
///
...
public final class Surveys
  ...
  /// @throws NullPointerException if signClass or pipe is `null`
  ...
  /// Returns a lookup that creates cached Survey instruments from a conduit.
  ///
  /// @param <S>       the Sign enum type from the source API
  /// @param signClass the class object for the sign enum
  /// @param conduit   the conduit providing signal pipes
  /// @return a lookup that creates Survey instruments
  /// @throws NullPointerException if signClass or conduit is `null`
